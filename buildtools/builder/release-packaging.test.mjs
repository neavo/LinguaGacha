...
describe("发布包构建配置", () => {
  ...
  it("GitHub Actions 只构建和上传当前平台架构的最终发布资产", () => {
    ...
    expect(workflow).toContain(
      "build/release/${{ needs.prepare.outputs.version }}/LinguaGacha_v${{ needs.prepare.outputs.version }}_Linux_x64.AppImage",
    );
    ...
    expect(workflow).toContain(
      "./artifacts/${{ needs.prepare.outputs.app_name }}_${{ needs.prepare.outputs.version }}_Linux_x64/LinguaGacha_v${{ needs.prepare.outputs.version }}_Linux_x64.AppImage",
    );
