...
describe("boundary-check core", () => {
  it("递归收集文件时跳过构建产物和依赖目录", () => {
    const project_root = create_temp_project();
    try {
      write_project_file(project_root, "src/main/index.ts", "");
      ...
      expect(files).toEqual(["src/main/index.ts"]);
    } finally {
  ...
  it("执行规则并格式化带行号的错误", () => {
    const project_root = create_temp_project();
    try {
      write_project_file(project_root, "src/main/index.ts", "bad_call();\n");
      ...
      expect(format_boundary_errors("示例检查", errors)).toContain(
        "[示例规则] src/main/index.ts:1 命中坏调用",
      );
    } finally {
      rmSync(project_root, { force: true, recursive: true });
    }
...
function create_temp_project() {
  return mkdtempSync(path.join(os.tmpdir(), "linguagacha-boundary-core-"));
}
