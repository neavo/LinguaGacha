...
const API_GATEWAY_RELATIVE_PATH = "src/main/api/api-gateway-server.ts";
const NATIVE_FS_RELATIVE_PATH = "src/native/platform/native-fs.ts";
      ...
      for (const file_path of context.files.filter(is_backend_production_source)) {
        ...
        for (const import_entry of find_import_specifiers(context.read_file(file_path))) {
          ...
          errors.push({
            ...
            message: "生产代码真实磁盘 IO 必须经 src/native/platform/native-fs.ts",
            relative_path,
          });
        }
      }
...
function is_backend_production_source(file_path) {
  return (
    ...
    (file_path.includes(`${path.sep}src${path.sep}main${path.sep}`) ||
...
function is_database_or_migration_path(relative_path) {
  return (
    relative_path.startsWith("src/main/database/") ||
    relative_path.startsWith("src/main/migration/")
  );
}
