...
function chunk_target_files(target_files) {
  const chunks = [];
  const chunk_size = 80;

  for (let index = 0; index < target_files.length; index += chunk_size) {
    chunks.push(target_files.slice(index, index + chunk_size));
  }
...
async function run() {
  ...
  const base_formatter_args = [
    ...(check_only ? ["--check"] : []),
    "--no-error-on-unmatched-pattern",
  ];
  const target_chunks = chunk_target_files(target_files);

  for (const target_chunk of target_chunks) {
    await execa("oxfmt", [...base_formatter_args, ...target_chunk], {
      cwd: process.cwd(),
      preferLocal: true,
      stdio: "inherit",
    });
