...
- Agent 与 OneShot 共用 [`BACKEND.md`](BACKEND.md) 定义的模型请求边界。模型配置中的 `agent.context_window` 与 `agent.max_output_tokens` 各自以 `0` 表示自动；每条 Agent 消息及手动压缩重试受理后、模型调用前按 `model_id` 解析领域规则或稳定兜底，并把生效容量、思考等级与压缩预留同步到既有 `AgentSession`。自动压缩在模型可见上下文达到 `context_window - max_output_tokens` 时触发，页面提前一份输出预算预警；压缩成功后 token 仪表直接采用 SDK 对新模型历史的估算，失败保留原用量。设置作用于同一对话的下一次模型操作，不重建或清空模型历史。模型页 generation 和 threshold 输入 / 输出 token 设置只作用于 OneShot。
