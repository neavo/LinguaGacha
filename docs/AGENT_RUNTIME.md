...
- System Prompt 规定如何匹配和组合技能，要求模型在执行相关任务前读取完整正文，并复用上下文中已有的正文。技能入口规定如何选择任务文件、判据和扩展技能。任务文件说明业务单元的对象与计数方式，以及默认范围、取证方法、后续调查、提交条件、验收要求和成果。各任务文件完整说明自身的领域流程，在关键步骤重申相关全局要求，并引导模型按需读取专业判据。
...
- 采用自启发调查的领域流程，在各自文件中完整描述冷启动、种子发现思路、逐轮调查和结束条件。模型通过 `workspace_run` 实时维护 JSON 种子账本，按以下约定记录和检查进展：
  - 在 `work/TASKNAME-seeds.json` 中记录 `completedRounds` 和 `seeds`。`seeds` 按目标轮次组织种子，每条种子记录 `pattern` 和 `consumed`。
  - 用 `completedRounds` 记录包含冷启动的已完成总轮数。冷启动完成后设为 `1`，后续每轮完成取证、结果处理和种子提炼后增加 `1`。
  - 在取证前保存种子，得出新增或修改种子的结论时立即写入账本。结束前重新读取账本，确认已完成至少两轮，且各轮的全部种子均已消耗。
  - 将取证记录、检查结果与领域成果保存在业务工作材料中，根据检查结果和覆盖证据决定继续调查、结束循环或报告受阻。
...
- System Prompt 规定通用决策规则、授权与可信边界，并要求修正后的参数仍表达实际意图。工具说明解释调用与恢复行为，Schema 规定参数约束，`ws.contract` 的主题参考定义对象结构、修改格式与副作用。`workspace_run` 的调用说明要求先读取目标 `reference`，其执行限制与可用包名来自运行策略和依赖清单。`workspace_apply` 的说明依据 `ws.contract` 生成，解释提交与回执的含义。
