...
- GUI Agent 在应用 userdata 中持有一个活动 UUID 工作区及其同级 `sources` 投影。工程加载时从 `.lg` 原始资产生成 sources，同一工程 epoch 与 files revision 的后续 workspace load 直接复用；files revision 变化后下一次 load 才替换投影。普通文本保留为单文件，EPUB / XLSX 保留容器内部路径并只展开文本成员；投影失败不回滚工程加载，由首次 workspace load 重试。显式 Agent reset 只清理活动工作区，工程切换、dispose 或下次应用启动清理 sources。新 load 全部成功后才替换旧工作区；每次成功 `workspace_script` 把当前 change 文件与 scratch 派生工作记忆事务提交到该工作区基线，脚本失败、取消或结果无效只回滚本次运行。工程身份 / 七个 section revision / 语言变化、reset、工程切换、apply 成功或无变化、stale、无法补偿的文件事务 / 未知宿主失败和 dispose 会清理活动工作区；change 校验和数据库事务回滚保留工作区供修复或安全重试。目录清理先解除活动身份，失败只留诊断并由下次启动清除，不改变项目提交事实；工作区与 sources 都不进入 Agent snapshot、模型历史或项目事实。
