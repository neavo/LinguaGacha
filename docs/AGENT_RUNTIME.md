...
- 工作区按业务领域相邻组织只读数据和 load 时证据；analysis 状态与候选不进入工作区，warnings 与 evidence 不随程序化处理重新计算。固定 change 文件按 items / prompts 更新和每个 quality kind 的创建、更新、删除、移动分开；当前快照下的派生任务工作记忆只写 `scratch/**`。具体路径、字段与 recipe 查询算法留在 contract、发布源码和行为测试。
