...
- JavaScript 是唯一处理编排面。`workspace.runRecipe(name, args)` 在脚本内调用 contract 声明的发布 recipe；recipe 只获得 contract、读取与列表 API，不能写 change、task、scratch 或递归调用 recipe。查询 recipe 读取当前快照，公共词根 recipe 只为调用方已经确认相关的显式词形枚举候选，不访问全局规则或推断语义关系；内置 recipe 都返回具名对象，参数、分页与输出限制以 contract 为准。
