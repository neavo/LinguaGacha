...
- 技能文件查询与写入由 `AgentSkillsService` 串行处理。来源入口可以是目录链接，命令在真实技能包目录内执行。内置技能只读，用户技能允许编辑和文件管理。包内链接不参与管理，`ui.json` 隐藏并受保护，根目录 `SKILL.md` 固定名称与位置。
- 保存以读取时的内容摘要检查版本，通过同目录临时文件替换正文。主文件保留额外 YAML 字段，将元数据后的空白行归为结构分隔，写回时固定一个空行。正文缩进及后续空白保持原样。
- 技能名称须与目录名一致。改名同步迁移目录、启用和排序偏好，失败时执行补偿。多文件修改不具备崩溃事务。当前会话沿用原目录绑定，新身份在下一会话生效。
