...
- GUI Backend 组合根向 Agent 和 Gateway 提供同一个 `AgentSkillsService`，由它持有当前可用技能集合。管理查询、修改与会话读取共用串行队列，首次受理在读取技能的同一操作内同步绑定。初始化、首次受理、重置和工程切换都会扫描技能。同一来源按稳定路径保留首个有效同名包，坏包和重复包进入日志。SDK 关闭默认工具和资源发现。
- `AppSettingService` 保存应用级 `agent_skills` 偏好并发布 `settings.changed`。关闭名称按来源保存，用户顺序按名称保存。内置顺序来自 `ui.json.order`，新用户技能按名称追加。
- 来源入口可以是目录链接，命令在真实技能包目录内执行。内置技能只读，用户技能允许编辑和文件管理。包内链接不参与管理，`ui.json` 隐藏并受保护，根目录 `SKILL.md` 固定名称与位置。
- 保存以读取时的内容摘要检查版本，通过同目录临时文件替换正文。主文件保留额外 YAML 字段，将元数据后的空白行归为结构分隔，写回时固定一个空行。正文缩进及后续空白保持原样。
- 技能名称须与目录名一致。改名同步迁移目录、启用和排序偏好，失败时执行补偿。多文件修改不具备崩溃事务。
- 管理页展示两个来源的公开技能。可用集合先过滤关闭项，再由用户包覆盖同名内置包。关闭用户包后回退到仍启用的内置包。隐藏资源继续参与会话加载。
- 空白对话跟随当前技能集合，通过保持 `sessionId` 的 `snapshot_seed` 更新 mention 候选并保留草稿。首次消息受理绑定技能，系统提示词、引用候选和读取工具共用这份集合。提交前失败或取消会释放候选运行时和绑定。已提交对话保留技能及目录绑定直到重置，期间的设置和身份变更由下一对话采用。
...
- [系统提示](../builtin/agent/system_prompt.md) 负责人格、任务与授权边界、技能选择、CodeAct、业务单元与提交、恢复和交付要求，创建模型会话时在正文末尾附加已绑定的技能目录。
...
- 消息级“复制”与“编辑”共用当前可修订消息的操作区；复制仅对其中有正文的 user / assistant 开放且不改变会话状态，输入消息的保存并重试会重新运行最新 round。历史 user、assistant 与队列项各自在目标位置展开独立编辑器，失败时保留编辑内容。assistant 编辑隐藏附件与 marker 能力。输入框、引导卡片与时间线通过共享语法解析器把引用投影为整块视觉，正文是唯一事实来源，识别不依赖当前候选列表。
