...
- Agent 与 OneShot 共用 [`BACKEND.md`](BACKEND.md) 定义的模型能力合成、六档降级和请求覆盖边界。模型配置中的 `agent.context_window` 与 `agent.max_output_tokens` 各自以 `0` 表示自动；每次 Agent 模型操作前把生效容量与思考等级同步到既有 `AgentSession`。Agent 固定保留 32K 上下文用于自动压缩；最大输出超过 `context_window - 32K` 时由统一模型边界自动调小，格式损坏或无法容纳固定预留时整组恢复为 `0/0` 自动配置，模型页同步回写规范值并显示同一条调整警告。页面从 `context_window - max_output_tokens - 32K` 起预警；设置作用于同一对话的下一次模型操作，不重建或清空模型历史。模型页 generation 和 threshold 输入 / 输出 token 设置只作用于 OneShot。隐藏“继续”消息在操作发起时按当前 `app_language` 解析。
...
- `translation-plan` 是只生成任务资产的公开入口：它必须读取当前获胜 skill 包内的 `references/rules.md` 作为唯一规则来源，从当前工作区排除 `PROCESSED` 与 `ERROR`、把其余候选状态统一投影为 `NONE`，并将完整翻译、跳过与复用计划写入当前对话 `task/translation-plan.json`；它不提交工程 change 或启动翻译任务。
...
- `workspace_load` 无参数生成完整只读快照和空 change 文件、挂载当前对话 task，只在工具结果返回语言与数量摘要；完整 project_meta 和 contract 保留在磁盘，脚本运行时把 contract 投影为 `workspace.contract`。`items/entries.jsonl` 直接携带 canonical `file_type`、`text_type`、`tag` 与 `extra_field` 只读格式事实，skill 直接消费 item 已有语义，sources 只补足缺失片段或结构证据；project_meta 保存解释快照所需的语言、数量、文件顺序及可用的 source 文本路径或容器文本根，质量规则功能开关和文本处理设置不进入工作区。contract 是 datasets、显式 change 操作、字段、身份、稳定写入副作用、领域提交软建议、模型结果与查询上限的唯一代码权威，不承载固定脚本 SDK 或运行时生命周期。`workspace_script` 的 TypeBox 工具 Schema 是固定 SDK、具名工作区方法和完整入口语法的唯一模型可见权威，Electron runner 注入相同成员。System Prompt 规定 items 优先、sources 仅补足缺失片段或结构证据，并提供无 skill 时读取工作区事实、准备并提交 contract 声明变更的完整默认流程；skill 只补充领域判断与处理方法。
