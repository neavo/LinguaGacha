...
| 当前完整数据工作区、快照与显式 change 准备         | `AgentWorkspaceService` | `workspace_load`、`workspace_script`、`workspace_apply`                     |
...
- `workspace_load` 无参数生成完整只读快照和空 change 文件，只在工具结果返回语言与数量摘要；完整 project_meta 和 contract 保留在磁盘，脚本运行时把 contract 投影为 `workspace.contract`。project_meta 保存解释快照所需的语言、数量与文件顺序；contract 是 datasets、显式 change 操作、字段、身份、稳定写入副作用、领域提交软建议、模型结果与查询上限、脚本 API、recipe 参数及具名返回形状的唯一代码权威，不承载运行时生命周期。System Prompt 提供无 skill 时读取工作区事实、准备并提交 contract 声明变更的完整默认流程，skill 只补充领域判断与处理方法。
...
- JavaScript 是唯一处理编排面。`workspace.runRecipe(name, args)` 在脚本内调用 contract 声明的发布 recipe；recipe 只获得 contract、读取与列表 API，不能写 change、修改 scratch 或递归调用 recipe。三个 recipe 都返回具名对象，参数、分页与输出限制以 contract 为准。
