...
- System Prompt 规定最小初探、通用任务边界、交互、证据复用、业务单元和后续调查准则。采用包内资源分派的技能，在入口中说明资源选择条件、必要初探和加载关系，在领域参考中说明具体意图、默认范围和执行条件。
- 包内判据规定对象资格、安全条件和指定操作的核验要求。领域流程说明目标解析、取证、后续调查、提交时机和验收成果。格式与文本质量参考提供专业知识，各资源在使用位置注明所需参考和脚本说明。
