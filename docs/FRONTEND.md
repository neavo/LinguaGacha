...
- 校对以 `kind` 区分文本和页面，共用远端列表窗口。文件选择由页面查询状态拥有，`default` 跟随工程文件，`selected` 保留显式集合（包括空集）；筛选弹窗只拥有文本内容条件草稿。文件与术语的局部搜索只改变候选显示，组选择作用于全部候选，术语组同时包含「无术语缺失」。查询和计数口径归 [BACKEND](BACKEND.md)。
