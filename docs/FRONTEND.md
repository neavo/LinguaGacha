...
- Agent 待决卡片独立挂载，主草稿和工具栏可用性保持各自的状态归属；共享编辑器提供正文与附件能力，主 Composer 和原位编辑器分别决定提交权限。
...
- 独立全量翻译（`source: standalone`、`operation: translate`、`scope.kind: all`）从活跃态进入 `done` 时请求导出确认；Agent 批量结果由工具承接后续步骤。页面与任务完成通知共用预检及确认流程，运行态不锁定导出。前往 Agent 时保留已有草稿，仅为空草稿填入审校建议。
