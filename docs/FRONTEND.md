...
- Agent Markdown 直接渲染原始 HTML 与图片，并保留默认 URL 过滤；非空相对链接经 Backend API 激活工作区目标，由后端决定保存文件或打开目录，外部链接交给宿主外部打开，页内锚点保留原生跳转，被过滤为空的目标显示文本。工作区链接约定归 [`AGENT_RUNTIME.md`](AGENT_RUNTIME.md)。
