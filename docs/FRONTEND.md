...
```mermaid
flowchart LR
  X["src/gui/bridge + ipc + shell-contract<br/>桌面宿主契约"] --> A["preload<br/>window.desktopApp"]
  X --> B["desktop-api.ts"]
  A --> B
...
- 项目变更按 payload mode 分流：canonical 行级 items/files delta 直接合并，完整 items/files 替换和其它 section 通过 `sections.*.data` 合并；校对保存、清空译文和状态设置可发布 `items.field-patch`，`ProjectStore` 只把后端确认的 `dst`、`status`、`retry_count` 合并到已持有完整 item DTO；任务翻译这类高频 item 变更可发布 ids-only，只在 renderer 刷新窗口合并 id 后补读公开 DTO；`section-invalidated` 只用于异常恢复。ids-only、`section-invalidated` 或缺少后端 section revision 的项目变更不能直接写入 `ProjectStore`；补读请求必须绑定发起时的 project path 与 epoch，读回后同时校验当前项目身份、响应 project path 与后端 section revision，项目切换、同路径重新 warmup 后的迟到响应或旧 revision 响应必须丢弃，合格响应再以 exact revision 合并。
- 项目变更中的 canonical item upsert 必须携带完整公开 DTO；只表达少量字段变化的路径必须走后端 `field-patch`，页面不能提交或制造瘦身 item DTO。
...
- Project UI Worker 中校对缓存身份必须同时覆盖 project path、source language、items / quality / proofreading revisions；items field-patch 只在 worker 内更新目标 id 的原始行和评估行，不从 ProjectStore 复制完整 DTO；当前列表 view 缓存必须携带筛选、搜索和排序上下文，按变更前后评估目标 id 是否保留、移除、插入或重排，批量重排只重建 worker 内派生 id 序列，不回读后端也不重建项目事实。
- 校对、质量规则和姓名字段等结果型页面的主列表使用结果视图快照：搜索、筛选、替换、排序或刷新等显式 action 生成新的有序稳定 id；项目事实刷新只能在当前 view 语义内更新受影响实体和窗口事实，质量规则新增、导入、重排、重置等改变成员或顺序的 action 以 mutation result 确认的项目身份和目标 section revision 作为重建门闩，无论 SSE 与 HTTP 先后，只有 `ProjectStore` 当前项目和 section revision 同时到达目标事实源后才重建快照；实体删除、项目切换、全量数据源重建或运行态不兼容时才剪除或重建快照，不按位置把旧 id 映射到新实体。
...
- `src/gui` 桌面宿主契约的桥接 API、IPC、标题栏壳层、Core API 地址注入或外链策略变化。
