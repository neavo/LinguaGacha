...
- 校对以 `kind` 区分文本和页面，共用列表窗口与状态展示。文件选择的 `default` 跟随工程文件，`selected` 保留显式集合及空集，筛选弹窗持有内容条件草稿。筛选弹窗内的候选搜索只影响显示，全选覆盖对应维度的全部候选，术语组包含「无术语缺失」。文件更新保留查询意图和阅读锚点，旧窗口失效后带锚点重查。工作台表头排序属于页面状态，拖动保存工程顺序。查询与计数口径归 [BACKEND](BACKEND.md)。
- 校对文件选择使用原始文件路径作为身份，目录勾选更新全部后代文件的显式集合。展开状态属于选择器本地状态，工程切换时重建。
