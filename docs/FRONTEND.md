...
本文件只回答 Electron / preload / renderer / 页面 query runtime / 导航 / 样式消费边界。后端协议权威归 [`docs/BACKEND.md`](BACKEND.md)，产品语义和视觉规范分别走 `PRODUCT.md` 与 `DESIGN.md`。
...
flowchart LR
  ...
  Runtime --> Store["TaskRuntimeStore<br/>project refresh signal"]
...
- `DesktopRuntimeProvider` 是主窗口内 settings、项目身份、task snapshot、项目 session 初始化、事件流和轻量项目刷新信号的唯一运行态入口。
