...
| 层 | 职责 | 不该做什么 |
| --- | --- | --- |
| `src/main` | Electron 宿主、窗口、标题栏、原生对话框、外链打开、开发态调试端口、内部 Database Service、Python Core 伴生进程生命周期 | 不持有页面状态，不组织页面业务 HTTP 请求，不写 React 逻辑 |
...
- `src/main/lifecycle/` 是 Python Core 伴生进程生命周期的唯一前端侧落点；Electron main 先启动 `src/main/database/` 内部服务并生成 token，再从启动根目录优先拉起平台 Core helper（Windows 为 `core.exe`，macOS / Linux 为 `core`），不存在时回退到 `uv run app.py`。
- `src/main/database/` 是 `.lg` SQLite、事务和 asset 读写的唯一物理存储实现；`src/main/utils/zstd-tool.ts` 是 Electron main 侧 Zstd 压缩等级、压缩与解压工具的唯一落点；`src/main/migration/project-database-migration-service.ts` 承接 `.lg` 打开期 schema 与旧物理格式迁移；内部服务只监听 `127.0.0.1` 随机端口，只接受 token 校验后的内部请求，不暴露给 preload 或 renderer。
...
### Core API 地址来源
- 应用正常启动时，Electron main 的 `CoreLifecycleManager` 会先启动内部 Database Service，再在高位端口范围内选择 Core 本机端口，从启动根目录启动 Python Core，校验 `/api/health` 返回的 Core API token，最后把实际地址写入 `LINGUAGACHA_CORE_API_BASE_URL`。
- `LINGUAGACHA_CORE_API_TOKEN` 只注入 Python Core 进程，用于 Electron main 调用内部生命周期接口和识别当前 Core 实例。
