...
- 启动时并行读取 `/api/settings/app`、`/api/session/project/snapshot`、`/api/tasks/snapshot` 作为初始状态；这一步不能通过关闭工程来“重置”后端会话。
- `TaskSnapshotStore` 只缓存 `TaskSnapshot`，并用 `run_revision` 丢弃旧 snapshot；task 不进入项目 query 或页面计算缓存。
- settings 只能由后端设置载荷同步，task 只能由后端 task 载荷或任务命令 ack 同步，project identity 只能由后端项目载荷同步。
- 页面写入只能提交用户意图、设置镜像和后端 query 返回的依赖 revision，并必须通过 `commit_project_write` 携带页面领域拥有的显式 `operation` 进入统一提交管线；页面不能自建刷新、诊断或写入结果应用链路，也不能把计算出的 items、task extras、prefilter config 或 analysis extras 当后端事实提交。
...
- 同步写入结果与 SSE 统一归一成轻量项目刷新信号；页面按自身 query 参数重新读取 query 结果，不能在前端合并项目事实。
...
- 前端消费的数据实体和值对象从 `src/domain` 导入；跨运行时纯规则、协议词表和工具从 `src/shared` 导入；最终项目写入计算算法只属于后端，前端不导入或复刻。
...
- 质量规则页面使用 `QualityRule` / `Prompt` 计算出的公开 key 与归一化切片；请求预设时传公开 rule type，不传物理目录名。
- 源语言与目标语言控件分别消费 `SOURCE_LANGUAGE_CODES` 与 `TARGET_LANGUAGE_CODES`；`ALL` 只作为源语言过滤关闭值，不进入目标语言控件。

## 5. 导航、Session 与页面计算缓存
...
- 页面计算缓存、弹窗、确认框、导入状态和提交中状态随页面挂载创建、随卸载释放；只有登记到 `ProjectSessionUiStateProvider` 的轻量页面 UI 状态可在当前项目 session 内跨路由保留。
- 工作台和校对页可以维护页面局部缓存，但 ready 判定必须基于项目 path、required sections 与 consumed revisions。
- 校对页搜索、筛选、排序、窗口和警告计算由后端校对 query 提供；前端只保存当前参数、view id、窗口结果、选择和编辑态，cache ready 与 consumed revisions 以后端 `sync` 返回的 section revision 为准，页面卸载或路由切换不代表后端校对页面读取缓存清理。
...
- 工作台统计区和任务菜单百分比消费后端 query 计算的 `WorkbenchStats`；任务详情在运行或停止中消费 `TaskSnapshot.progress`，空闲态才回落到 `WorkbenchStats`。
...
- 前端静态检查会拦截可见中文硬编码、后端 API 直连、GUI 契约越界、共享状态内部写入口越权、废弃 `hooks` / `lib` 入口、`widgets/interactions` 越权、`--ui-*` token 越界和 rem 尺寸字面量。
...
- 改 `app/state`、导航注册、`ProjectSessionProvider`、页面计算缓存、项目 UI 状态、后端校对 query 消费方式或质量规则统计共享缓存策略，更新本文。
