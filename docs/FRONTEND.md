...
| 层 | 职责 | 不该做什么 |
| --- | --- | --- |
| `src/main` | Electron 宿主、窗口、标题栏、原生对话框、外链打开、开发态调试端口、公开 TS Gateway、内部 Database Service、Python Core 伴生进程生命周期 | 不持有页面状态，不写 React 逻辑 |
...
- `LINGUAGACHA_CORE_API_TOKEN` 只注入 Python Core 进程，用于 Electron main / TS Gateway 调用内部生命周期与 `/internal/llm/request` 窄路由，不暴露给 renderer。
...
- 它看到的 baseUrl 永远是 TS Gateway；Python Core 内部 baseUrl 和 token 不进入 `window.desktopApp`。
...
- `src/main/log/` 是 Electron main 的日志权威：控制台输出、`DATA_ROOT/log/app.yyyymmdd.log` 文件输出和日志窗口 ring buffer 都由 TS `LogManager` 分别按目标开关管理；磁盘日志只保留日期最新的 3 份；Python `LogManager` 只保留兼容调用口，通过 TS Gateway 的 `/api/logs/append` 把结构化日志交回 TS。
...
| `ts-conversion` | `pages/ts-conversion-page/page.tsx` | 从 `ProjectStore.items / quality.text_preserve` 本地派生转换结果；OpenCC、文本保护分段和内置预设读取在 TS 侧执行，Python Core 只保留转换结果文件写出 |
