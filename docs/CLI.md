...
本文件是 CLI 入口分发、命令协议、临时工程、外部资源、输出语义和平台启动器的唯一归宿。后端 API、数据库和任务引擎规则只在 [`docs/BACKEND.md`](BACKEND.md) 展开。

## 1. 入口边界

- CLI 只能由产品入口中的显式 `--cli` 触发；用户参数从 `--cli` 后开始读取，平台启动器不得把文件名或进程名语义泄漏给 `src/index.ts`。
- 产品入口负责解析 appRoot 和桌面 bundle 根，并把 `BackendWorkerExecution` 原样传给 CLI；CLI、后端组合根与各 worker 消费方不探测 worker 文件，也不把 `in_process` 当失败回退。
- CLI 以 `BackendBootstrap(exposeApiGateway=false)` 启动后端组合根，不开放本机 HTTP / SSE Gateway；后端控制台日志在 CLI job 中关闭，避免诊断文本污染 stdout JSONL。
- CLI 是文件进出型适配层：它只接收输入文件、输出目录、语言和本次命令显式资源，不承接 GUI 工程文件心智或前端协议。
...
sequenceDiagram
  ...
  participant Backend as BackendBootstrap
  ...
  Entry->>CLI: --cli 后 argv + appRoot + BackendWorkerExecution
  CLI->>Backend: exposeApiGateway=false
  Backend-->>CLI: BackendServices
  CLI->>Job: run_cli_job
...
- `progress.stats` 是 CLI 对外稳定组装，字段为 `total`、`skipped`、`failed`、`completed`、`pending`、`percent`；不要把内部 `TaskSnapshot.progress` 字段名直接暴露成 CLI 协议。
- 成功的 `finished` 不再重复输出产物路径，调用方以自己传入的 `--output-dir` 作为产物位置；失败的 `finished` 携带稳定 `error.message`，进程返回运行期错误码。
- stderr 只承载参数错误、帮助回显或运行期错误；后端诊断日志写入日志文件，不进入 job stdout。
- CLI 完成后 `src/index.ts` 必须按返回码主动结束 Electron 进程，避免 Windows console launcher 等待未退出子进程。
- CLI 不打开输出目录；`openOutputFolder` 在 CLI BackendBootstrap 中固定为空操作。
...
- 改 `--cli` 分发、appRoot 解析、CLI 进程退出、`BackendWorkerExecution` 下传或平台启动器，更新本文。
- 改临时工程生命周期、默认预设关闭策略、资源写入、revision 推进、任务等待或导出链路，更新本文。
- 改后端 API、任务、数据库或 worker 执行契约，同步 [`docs/BACKEND.md`](BACKEND.md)；改验证要求，同步 [`docs/WORKFLOW.md`](WORKFLOW.md)。
