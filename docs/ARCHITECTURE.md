...
- GUI main 与 Backend Runtime 只交换 `src/shared/backend-runtime.ts` 定义的结构化控制协议和工作区 Node 预加载模块路径：ready、stop、语言读取、宿主诊断，以及代理解析、打开目录和选择保存路径的宿主回调（导出与 Agent 共用目录打开能力）。宿主操作以 requestId 隔离并发；取消或 runtime 关闭立即结束 worker 的待决等待，迟到的宿主回包直接丢弃。Agent 工作区子进程与代理通信归 [`AGENT_RUNTIME.md`](AGENT_RUNTIME.md)。worker 意外退出直接结束应用，不回退同进程、不自动重启。
