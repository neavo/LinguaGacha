...
- `frontend/src/main/api/` 是 Electron 运行时公开 `/api/*` HTTP / SSE 编排入口；`project/` 承载已迁移项目轻生命周期、项目同步 mutation、reset preview、公开 bootstrap 运行态编码、`project.patch` 补全和 section revision 口径；`task/` 承载公开任务命令、任务回执、任务运行态和 task snapshot；`task-engine/` 承载后台任务生命周期、调度、限流、停止、重试与提交循环；`task-worker/` 承载单个 work unit 的确定性文本处理、prompt、响应解析和 Python LLM adapter 调用；`file/` 承载公开文件解析 / 写回、create preview 草稿解析和导出目录语义；`model/` 承载模型页快照、CRUD、重排和激活回退；`service/` 承载应用设置、质量规则 / 提示词、校对同步保存与 Electron main 运行期路径规则。未迁移业务由 API 编排层代理到内部 Python Core。
- `api/` 保留 Python Core 的内部协议、事件、`/internal/llm/request` LLM SDK adapter 和 Python 客户端对象化契约；公开项目轻生命周期、bootstrap、文件预演 / 导出、reset preview、任务协议、任务执行态、section revision 和前端运行态 patch 编码权威在 TS Gateway，不再由 Python Core 公开承载。
- `module/Data` 持有 Python 数据领域兼容门面，`module/Engine` 保留 `TaskRequester`、模型请求流和模型探测能力，`module/Model` 持有模型配置领域规则。
...
| API -> Task Engine / Task Worker / Python LLM adapter | 后台任务启动、停止、调度、进度与终态语义由 `frontend/src/main/task-engine` 提供；单个 work unit 的确定性处理由 `frontend/src/main/task-worker` 提供；Python Core 只通过 `/internal/llm/request` 执行真实 LLM SDK 请求并返回原始请求事实 | 防止数据层、界面层或 Python Core 偷持任务生命周期或并行解析业务结果 |
| File format | 公开文件解析与写回统一落在 `frontend/src/main/file/`，包括 EPUB AST / legacy 写回兼容 | 防止格式实现分散到 Python Core 或 API 编排层 |
| Data -> Database | TS Task Engine 通过 `TaskDataService` 读写任务数据并提交项目事实；Python 兼容数据门面只通过 `module/Data/Database/DatabaseGateway.py` 调 Electron main 内部 database workflow；SQL / 事务 / `.lg` asset 读写只落在 `frontend/src/main/database/`，Zstd 压缩参数与压缩 / 解压工具只落在 `frontend/src/shared/utils/zstd-tool.ts`，`.lg` 打开期 schema 与旧物理格式迁移统一落在 `frontend/src/main/migration/project-database-migration-service.ts` | 防止事务、schema 与压缩格式在 Python / TS 两侧并行 |
...
| `frontend/src/main/task` | 公开任务命令、内部任务数据路由、事件 hub、任务运行态、任务回执和 task snapshot 组装 | `frontend/src/main/api`、`frontend/src/main/project`、`frontend/src/main/database`、`frontend/src/main/task-engine` | [`API.md`](./API.md)、[`DATA.md`](./DATA.md) |
...
| `frontend/src/main/task-worker` | 单个翻译 / 分析 / 重翻 / 低频单条翻译 work unit 的文本处理、prompt、响应解码、校验和 Python LLM adapter 调用 | `frontend/src/main/task-engine`、`frontend/src/shared/text`、`api/` | [`API.md`](./API.md)、[`DATA.md`](./DATA.md) |
...
| `frontend/src/main/model` | 模型页快照、CRUD、重排、激活模型回退与配置读取 | `frontend/src/main/api`、`frontend/src/main/service` | [`API.md`](./API.md)、[`DATA.md`](./DATA.md) |
...
| `module/Engine` | LLM 请求封装、请求流、模型探测与 Python 兼容运行能力 | `api/`、`module/Model` | [`DATA.md`](./DATA.md) |
