...
flowchart LR
  ...
  Services --> Cache["src/backend/cache<br/>session 热读缓存 / 结果读取缓存"]
...
- `src/backend/worker` 只承接非 engine 的无状态后台计算；engine planning / work-unit worker 仍归 `src/backend/engine`。
- `src/domain` 承载跨层实体和值对象的 JSON 边界、合法值集合和贴身判断规则；它不得反向依赖 backend、frontend 或 Electron。
...
5. 前端由 `desktop-api.ts` 探测 `/api/health` 后进入状态初始读取。
...
- 前端用 session manifest 校验项目身份，页面通过各功能域 query API 读取自身 query 结果；前端不再维护完整项目事实缓存。
...
| `src/domain` | 基础实体、值对象、合法值集合、贴身判断规则 | 页面策略、服务编排、传输外壳 |
...
| `src/backend/project` | loaded 工程身份、生命周期、无状态项目数据读取、committed event 与公开项目变更事件 | 页面局部状态、各领域写入和计算规则 |
| `src/backend/cache` | session 热读缓存管理、item / file / quality / prompt / analysis 子缓存、校对列表与质量统计结果读取缓存 | 项目写入、API 响应壳、engine 局部 LRU |
| `src/backend/{workbench,proofreading,quality,analysis,translation,toolbox,model}` | 项目领域服务；workbench 管结构性项目写入，translation 管译文文件产物导出，toolbox 管工具箱分支功能，其它领域只管各自 query、写入或计算 | Gateway 外壳、项目 session/cache/event 总线、任务引擎 |
...
| `src/backend/engine` | 翻译 / 分析任务引擎、任务状态、规划、执行、artifact 写回 | provider SDK 细节、页面结果缓存、项目 query 计算规则 |
