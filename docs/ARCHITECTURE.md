...
| Electron / preload / renderer / 页面 query runtime / 导航 / 样式消费 | [`docs/FRONTEND.md`](FRONTEND.md) |
...
1. `src/index.ts` 解析桌面 bundle 根目录，构造 `worker_threads` Core worker 执行配置。
...
| 层 / 模块 | 固定职责 | 不承接 |
| --- | --- | --- |
| `src/index.ts` | GUI / CLI 分发、appRoot 与 bundle 根解析、`CoreWorkerExecution` 注入 | 业务服务、命令语义、窗口状态 |
...
- 改 preload、`desktop-api.ts`、页面 query runtime、导航、项目页 runtime 或样式消费边界，更新 [`docs/FRONTEND.md`](FRONTEND.md)。
