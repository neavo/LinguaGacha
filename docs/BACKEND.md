...
- 质量规则的匹配语义集中在 `src/shared`：字面量执行 NFKC，`case_sensitive` 控制大小写折叠，正则遵循 JavaScript 原生语义。术语分别匹配 `src/name_src` 并检查对应译文字段，领域结果保存术语身份与字段归属。高亮、替换和包含关系按需计算字符坐标。正文替换与保护分析按行执行，姓名保护样例按整段提取；导入身份和包含关系共用相同匹配语义。
- `builtin/text_preserve/preset/*.json` 是内置文本保护规则的唯一内容来源；`base.json` 在所有模式下启用，其余预设按 `text_type` 提供智能规则，`custom` 叠加项目规则。
- 翻译与校对通过 `shared/text/translation-source` 先隔离资源，再准备正文行和整段姓名。`prepared_text` 用于请求与保护比较，`restoration_text` 保留译前替换前的形式。替换引入换行时，校对按组合正文重新分析；行数检查比较原稿与最终译文。空白行和完全保护行保留原文；全部行受保护时跳过请求，响应行数对应时恢复原始空白。
- 资源按输入顺序投影为 `lg-uri/<n>`，用于请求与 token 估算，译后按映射还原。校对比较真实保护值的顺序、数量及正文行号；重叠保护段沿用统一裁决。自然文本用空白覆盖保护范围并保留换行，维持词与资源边界。临时映射随计算使用，存储、警告和例句输出真实引用。
...
- 标点结构检查比较准备后的源文与最终译文，剥离保护片段及资源引用后按整段字段核验；允许同类形式转换，保留原文未闭合结构。分组比较不识别同组引号错配或符号相对正文的位置变化。条目警告、筛选与统计共用警告词表顺序。
- 外文残留、文本保护和标点检查覆盖正文及可见姓名，姓名数组读取第 0 槽。姓名先隔离资源，再整段分析；相似度、行数和译前处理作用于正文。这些检查要求对应译文非空，另共用条目状态门槛。术语逐字段比较，重试阈值属于条目。
- `warnings` 按规则和目标字段保存唯一结果，保护差异分别保留源侧与译侧证据；`glossary_applications` 保存术语应用事实。窗口和 Agent 输出复制嵌套证据。列表与统计按条目、规则去重，展示沿用规则词表顺序，同规则下正文在前。
- `FileCache` 向工作台和校对提供工程文件顺序，历史条目独有路径按首次出现顺序补在资产文件之后。校对列表、上下文与警告查询按文件顺序、行号或原稿页码、数值文本 ID 排列，列排序同值时沿用自然顺序。
- 校对读取器分别保存文本评估与页面事实，用带 `kind` 的行引用查询，在窗口响应边界生成展示字段。计算线程只返回文本 ID 和评估结果，正文由同次输入快照提供。`view_id` 标识稳定结果快照，正文与状态更新保留成员与顺序，删除剪除成员，文件同步、条目文件归属变化或内部文件候选增删使旧视图失效。失效窗口返回空 `view_id`，供前端识别并重查，包括原本为空的视图。新查询重新筛选和排序，上下文与按 ID 读取保留当前视图。
- 校对文件身份由原始 `file_path` 和 `internal_file_path` 共同确定，`null` 表示没有内部路径。`ProofreadingCache` 统一提取格式内部路径，列表、详情与筛选消费同一运行态。读取器在结构变化时按工程文件和条目自然顺序生成叶子候选及计数，正文更新复用候选。空外层文件保留零条目叶子。
- 校对查询的 `files` 直接传递 `default` 或 `selected` 意图，默认跟随全部候选，显式集合按完整文件身份匹配。列表与筛选计数共用匹配入口，Agent 的外层文件范围在当前快照中展开为叶子。
...
- `QualityRuleStatisticsCache` 统一提供命中数、代表例句和字面包含关系。命中缓存只读取结果及修订；规则变化同时失效统计与包含关系，文本变化按扫描范围失效统计，范围不明确时全量失效。正文译文变化只影响译后替换统计；译名、状态和重试变化可复用统计。
- `hits` 按当前已保存文本统计各规则独立匹配的条目数。术语扫描原始正文和可见姓名；替换扫描对应正文行的资源外片段；保护规则扫描投影后的原正文行和整段原姓名，可匹配包住资源的结构。临时标记内部命中不计数。worker 同次扫描选取最多两个稳定例句，保留上下文并还原引用。
...
- 本轮 `run_progress` 由 Runtime 持有，目标总量在执行前冻结，提交成功后累计终态条目和用量，终态保留，工程切换清空。批次允许仅提交用量，内容重试独立调度。工程累计 `progress` 持久化在 `translation_extras`，`ProjectWriteStore` 在事务内按条目真实状态变化维护行数。仅用量和同值批次只写进度，实际条目变化才推进项目修订并发布事件。Runner 提供耗时和用量，从已提交元数据读取工程计数。提交后同步失败保留已完成事实与原始诊断。
...
- 翻译任务以条目为请求、响应和提交单位。普通模型每条使用一条 JSONL 记录（`id`、`text`，姓名模式再加 `actor`），正文可包含换行。请求内按实际记录从 0 分配 `id`，独立于数据库条目 ID，响应按原值匹配并允许乱序。唯一匹配的非空译文独立提交，缺失、重复、未知或空白正文只影响对应条目。请求失败、零有效译文、部分有效和全部有效分别记录 `error`、`error`、`warning` 和 `info` 日志。worker 负责逐行准备与恢复，换行变化时保留完整译文，由校对派生 `LINE_COUNT_MISMATCH` 警告。
- SakuraLLM 按通用容量和文件边界合批，专用提示词将本批正文按换行拼接。解码器按实际请求正文行数恢复多个条目，空行参与对应。总行数不符时缩段重试，单条接收非空完整响应，允许内部换行变化。行数一致只提供位置对应依据，同时合并和拆分可能造成语义错位。
