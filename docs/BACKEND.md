...
- 校对读取器分别保存文本评估索引和页面索引，在响应边界生成展示字段，共用列表窗口及反向索引。计算线程只返回文本 ID 与评估事实，正文与请求身份由同次输入快照提供。默认列表按文件名、行号或原稿页码和字符串 ID 排列，文本上下文按工程文件顺序、行号和数值 ID 排列；页面译文列只提供查看入口，其排序沿文件和页码兜底。
- `view_id` 标识 GUI 稳定结果快照。字段更新保留成员与顺序，删除移除对应成员，新查询重新筛选和排序；上下文与按 ID 读取保留当前视图。
- `outcomes` 和术语条件只筛选、统计文本条目，页面只受文件范围及译稿搜索影响。结果计数仅按文件范围筛选，术语计数忽略自身条件；额外选项来自当前文本结果，文件候选及固定数量来自工程文件清单。真实警告摘要只统计成功文本的警告，无警告不保证内容正确。
- 热查询只比较工程、会话世代、修订号与处理配置，完整文本及质量输入在缓存未命中时构造。页面由 `ProjectDataReader` 按独立 `pdf` 修订号补读，不触发文本评估；迟到同步不能恢复已撤销的工程运行态。
