...
- `ApiGatewayServer` 是 Electron 运行态公开 `/api/*` 的唯一装配点；具体路由只允许注册在 `src/core/api/routes/*.ts`，POST JSON 路由必须经 `postJson` 返回统一响应壳。
...
- CLI 不启动 Gateway；CLI 命令通过同进程 `CoreServices` 和 `ApiStreamHub` 复用同一业务链路。
...
| 当前 loaded 工程热读数据 | `ProjectDataCache` | `ProjectLifecycleService` 热机、`ProjectEventBus` committed event、`/api/project/query/*` |
| Core 内部 committed event | `ProjectEventBus` | 写侧事务成功后的 after-commit 发布 |
| 项目公开 manifest | `.lg` + `ProjectDataReader` | `/api/project/manifest` |
| 页面 view model | `WorkbenchQueryService` / `ProofreadingQueryService` / `QualityStatisticsService` / `NameFieldExtractionService` + `ProjectDataCache` | `/api/project/query/*` |
| 同步项目写入 | `ProjectChangeCoordinator` + `WorkbenchService` / `ProofreadingService` / `QualityService` 等领域服务 | database transaction + internal event + `ProjectChangePublisher` |
...
- `/api/project/query/*` 是 renderer 读取项目 view model 的唯一公开入口；query response 必须携带本次 view 依赖的 `sectionRevisions`，页面写入使用这些 revision 作为乐观锁依赖。
- `ProjectDataCache` 是当前 loaded 工程的热读事实拥有者；query service 只能从 cache、按需数据库数据读取和 shared 纯算法组合页面 view，不能建立第二套长期项目事实缓存。
- 校对列表派生由 `ProofreadingCache` 在 Core 主进程持有；`ProjectDataCache` 不保存列表、筛选或 worker 细节，列表查询继续走本地 cache service。
- 非 engine 的重型派生通过 `CoreWorkerClient` 提交无状态 Core worker task；worker 不读数据库、不写 `.lg`、不发布事件、不持有项目 cache。
...
- 同步项目写入成功返回 `ProjectMutationResult = { accepted: true, changes }`；`changes` 与后续 SSE 广播是同一批后端 canonical `ProjectChangeEvent`。
...
- 后端写入不接收 renderer 派生的 `items`、task/progress extras、prefilter config 或 analysis extras 作为最终事实；页面只能提交用户意图、设置镜像和当前 section revision 依赖。
...
- 新增或改变 `/api/*` 路由、`src/core/api/routes` 注册边界、响应壳、错误投影、SSE topic、CORS 或 Gateway 生命周期，更新本文。
