...
- `ProjectWriteStore.apply_agent_workspace_changes` 是工作区唯一物理写入口：在 `BEGIN IMMEDIATE` 内读取当前目标、按对象 `fp` 重算 resolver，并将合法 item、PDFDocument、quality、prompt 尽可能一次提交。Item 显式变化与受影响重复组的被动变化形成同一实际变化集合，共同驱动写入、翻译统计、revision、cache 和 canonical delta；对象冲突形成逻辑部分成功，quality 每个变化 kind 只写一次并共享一次 aggregate revision，实际变化 section 才推进 revision。
...
- `pdf_documents(file_path, data)` 保存严格 Schema 的文档 JSON，原始字节保存在 assets。导入提交校验原稿 SHA-256，拒绝解析后变化的源文件；两者由同一事务写入。文字、字体和坐标提取作为可重新生成的工作材料，不进入文档存储。
...
- 当前 schema 创建 items、assets、pdf_documents、rules 与 meta；历史工程中已停用能力的表、规则与 meta 保留物理原值，当前 manifest、section、提示词与运行快照只投影现行事实。翻译提示词的路径和存储键由 `TRANSLATION_PROMPT` 固定描述对象拥有。
