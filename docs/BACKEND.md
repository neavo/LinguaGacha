...
- `ProjectChangeEvent` 的 payload mode 只有 `canonical-delta`、`ids-only`、`section-invalidated`；`items` / `files` 的 canonical 行级 upsert 由 adapter 按 id / path 从数据库投影回读，完整替换和非行级 section 在 canonical-delta 下由 `ProjectRuntimeProjectionService` 返回完整 section data，调用方不能手拼公开 data。长流程翻译可发布 items ids-only 事件，由 renderer 批量补读公开 item DTO；正常 mutation 不发布 `section-invalidated`，它只保留给异常恢复或无法携带 canonical payload 的事件。
