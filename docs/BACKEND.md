...
- Agent 产品工具只从当前设置、cache 与领域 query 读取事实，写入经对应服务的 Agent 专用入口复用同一 revision、事务和项目事件边界；质量规则统一查询、按条目形状分别写入，且不允许 Agent 修改启用状态或文本保护模式。`query_project_meta` 按需读取当前权威源语言和目标语言。模型可见的项目工具结果不暴露绝对 `projectPath`，item、warning、quality 查询及写入确认继续携带与数据快照绑定的 `sectionRevisions`。
