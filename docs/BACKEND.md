...
- Markdown 文本统一由 Markdown V2 的 AST 块 reader / writer 处理：`.md` 生成 `file_type: MD_V2`、`text_type: MD` Item；文本型 `.pdf` 先由本地 pdf-inspector WASM 提取 Markdown，再生成 metadata 同形的 `file_type: PDF`、`text_type: MD` Item。`row` 是块起始物理行，并与 `file_path` 共同作为 reset 重解析身份；格式 metadata 只保存块间布局，链接、图片与定义的 destination 投影为短资源引用，writer 再从原始 asset 宽松恢复。PDF reset 从原始 asset 经同一转换链重建；混合 PDF 保留 converter 返回的可用 Markdown，并由 reader 结果携带 `pagesNeedingOcr` 对应的跳过页；没有可用文本或 PDF 受密码保护时统一返回 `file.parse_failed`，diagnostic context 保留 `no_extractable_text` 或 `encrypted` 原因。
...
- 翻译 work-unit 以 item 为唯一请求、响应和提交单位：普通模型每个请求 item 使用一条 JSONL 记录（`index`、`text`，actor 模式再加 `actor`），`text` 可包含换行；SakuraLLM 每个 work-unit 只发送一个 item，并以完整纯文本承载译文。worker 内部才保留逐行准备与恢复事实。响应按 item index 独立裁决，缺失、重复、未知或空白正文只影响对应 item，请求错误 / 超时只影响本次发送的 items；结构变化的译文保留模型完整文本并由校对实时派生 `LINE_COUNT_MISMATCH` warning。
...
- `LLMClient` 独立拥有 OneShot 的总时限、取消和结果语义：供应商错误、长度截断、工具调用或空正文统一成为当前请求错误，任务层不解析异常文本；成功 usage 归一为输入、思考与输出三个互斥口径并分别进入任务快照。
