...
- 外文残留按目标语言允许的书写系统检查剥离保护片段后的译文，以完整 Unicode 字素簇保留附标；仅含一个 Latin 字素或恰好由两个 ASCII 大写 Latin 字母组成的残留片段不触发警告，其它 Latin 片段及其它书写系统残留继续报告。校对 worker 与 cache identity 携带完整文本处理配置，增量评估沿用全量同步冻结的配置。
