...
- 校对读取器用一个条目索引保存输入与评估结果，在响应边界生成展示文本。计算线程返回条目 ID 与评估事实，正文和请求身份由同次输入快照提供。默认列表按文件名、行号和字符串 ID 排列，上下文按工程文件顺序、行号和数值 ID 排列。
- `view_id` 标识 GUI 稳定结果快照。字段更新保留成员与顺序，删除事件移除对应成员，新列表查询重新筛选和排序。`row_id` 与反向索引支持窗口定位，上下文与按 ID 读取共用评估运行态并保留当前视图。
- 校对以 `outcomes` 表示翻译结果：条目分为“翻译成功 / 尚未完成 / 无需翻译”，成功条目再分为无警告或一个以上检查结果。条目更新维护结果、文件与术语计数，供默认筛选和警告摘要使用。筛选面板统计结果维度时仅按文件范围筛选，统计文件和术语维度时分别忽略自身条件。
