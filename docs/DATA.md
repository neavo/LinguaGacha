...
| --- | --- | --- |
| 任务工程上下文与任务数据 | `frontend/src/main/task/task-data-service.ts` | TS Task Engine 直接读取 / 提交；`/internal/task-data/*` 只作为 TS Gateway 受保护内部路由，不是公开协议 |
...
- Python Core 不直接导入 `sqlite3`，不理解 `.lg` 内压缩格式；任务热路径由 TS Task Engine 通过 `TaskDataService` 调 database workflow，仍需兼容旧数据门面时只通过 `module/Data/Database/DatabaseGateway.py` 调内部 HTTP database workflow。
...
### 引擎域
- TS `TaskRuntimeState` 是公开全局忙碌态的唯一权威来源；Python `Engine.status` 只保留内部兼容状态，不再服务整场任务调度，也不被公开 task snapshot 反查。
...
- `DataManager`、`ProjectSession`、TS Task Engine、`FileFormatService`、`Manager.py` 的权威入口变化
