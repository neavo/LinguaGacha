...
| 领域 | 权威入口 | 稳定职责 | 不该做什么 |
| --- | --- | --- | --- |
| `module/Data` | `DataManager.py` | 工程事实、规则、分析、翻译结果、校对辅助、Extra 数据能力、项目运行态编码 | 不持有后台任务执行骨架，不散落 SQL |
...
flowchart TD
    ...
    A --> C["Engine"]
    B --> D["Project / Quality / Analysis / Translation / Proofreading / Extra"]
    B --> E["FileManager"]
...
| 校对保存、重检、重翻、校对 revision | `Proofreading/*` | `ProofreadingAppService` 组合这些服务 |
| 繁简转换、姓名字段、导入术语辅助 | `Extra/*` | `ExtraAppService` 组合这些服务 |
