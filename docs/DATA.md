...
| `frontend/src/main/file` | `FileFormatService`、`FilePreviewService`、`FileExportService` | 公开非 EPUB 文件格式分发、解析、导出写回、工作台 parse 预演与新建工程草稿解析 | 不直接持有 SQL，不绕过 Database Service，不承载 EPUB 实现 |
| `module/File` | `FileManager.py`、`EPUB/` | EPUB 文件解析 / 写回和 Python 内部兼容调用 | 不再作为公开非 EPUB 文件能力 fallback，不持有工程生命周期、事务与项目加载态 |
...
flowchart TD
    ...
    B --> E["TS FileFormatService / EPUB FileManager"]
    B --> F["DatabaseGateway"]
...
| reset preview 公开预演 | `frontend/src/main/project/project-reset-preview-service.ts` + `frontend/src/main/database/` + `frontend/src/main/file` | TS Gateway 计算公开响应；翻译 all 预演直接解析非 EPUB asset，仅 EPUB 通过受保护 Python 解析桥 |
| 规则、提示词运行时读取 | `Quality/*` | Python Core `DataManager` |
| 分析候选、checkpoint、分析结果 | `Analysis/*` | `DataManager` |
| 校对保存、校对 revision、重翻提交 | `frontend/src/main/service`、`Proofreading/*` 与 `Engine/Retranslate/*` | 校对同步保存由 TS Gateway 调用 proofreading 服务直接写 `.lg`；Python 保留校对 revision 读侧与重翻任务链路，重翻由 `TaskAppService` 启动 Engine 任务并回写数据层 |
| 全局忙碌态与实时请求数 | `Engine.status`、`request_in_flight_count` | `Engine` |
| 非 EPUB 文件格式解析与写回 | `frontend/src/main/file` | TS Gateway 的文件预演、reset preview、导出服务 |
| EPUB 文件格式解析与写回 | `module/File/EPUB` | 受控 Python bridge |
...
- 如果它是非 EPUB 格式识别、提取条目或写回目标文件，属于 `frontend/src/main/file`；如果它是 EPUB 格式能力，属于 `module/File/EPUB`。
...
flowchart TD
    ...
    B -->|非 EPUB| C["TS FileFormatService"]
    B -->|EPUB| D["Python EPUB FileManager"]
    C --> E["具体格式处理器"]
    D --> E
...
稳定事实：
- 公开 `/api/project/create-preview`、`/api/project/workbench/parse-file`、translation reset all 的非 EPUB asset 重解析、`/api/tasks/export-translation` 与 `/api/project/export-converted-translation` 的非 EPUB 文件能力由 TS `frontend/src/main/file/` 持有。
- EPUB 暂留 Python `module/File/EPUB`，由 TS Gateway 通过受控 Python bridge 调用；工作台源文件预演、loaded 工程 asset 重解析与导出写回分别走窄内部桥，导出目录仍由 TS 文件导出服务一次性决定。这不是已迁非 EPUB 格式的 fallback。
...
迁移目录只承接会写回旧 userdata、旧配置事实或 `.lg` 打开期旧物理格式的行为；`.lg` schema 与旧物理格式读取兼容留在 Electron main 内部，具体规则统一放在 `frontend/src/main/migration/project-database-migration-service.ts`。payload 归一和文件格式 fallback 保留在原领域，例如 `Item/DataManager` 的状态边界归一、TS 文件域的非 EPUB fallback，以及 Python EPUB writer fallback 都不是迁移入口。
...
| 非 EPUB 公开入口 | 已迁到 TS `frontend/src/main/file/`，不再通过 Python `FileManager` 作为 fallback |
...
| 非 EPUB 文件解析中间态、写回兼容逻辑、格式判定顺序 | `frontend/src/main/file` | 保持已迁格式规则集中 |
| EPUB 解析中间态、写回兼容逻辑 | `module/File/EPUB` | EPUB 尚未迁移，保留 Python 权威 |
...
- `DataManager`、`ProjectSession`、`Engine.status`、`FileFormatService`、`FileManager`、`Manager.py` 的权威入口变化
