...
## 一句话总览
LinguaGacha 的数据域由 Electron main TS API 编排层、TS 任务数据 / 运行态服务、Electron main Database Service 的 `.lg` 物理存储，以及 Python Core 的无头 Engine 共同协作。本文回答的不是目录长什么样，而是：`Data / Engine / File / Model` 分别拥有什么权威职责，项目级状态应该落在哪里，唯一写入口怎么判断，`.lg` 物理存储为什么只由 Electron main Database Service 持有，以及哪些非显然规则会影响未来维护。
...
| `module/Engine` | `Engine.py` + `TaskRunnerLifecycle.py` + `TaskDataClient.py` | 后台任务生命周期、请求调度、停止、重试、进度、终态语义，以及通过 TS Gateway 读取 / 提交任务数据 | 不持有工程真相，不直接定义 HTTP 壳，不直接操作 `DataManager` 或 database workflow |
| `frontend/src/main/task` | `TaskService`、`TaskDataService`、`TaskEventHub`、`TaskRuntimeState`、`TaskSnapshotBuilder` | 公开任务命令、内部任务数据读写、事件 hub、任务回执与 task snapshot；持久进度读 database，实时状态读 TS runtime state | 不执行 Engine 调度，不绕过 database workflow，不把内部 token 暴露给 renderer |
...
| --- | --- | --- |
| Python 任务工程上下文与任务数据 | `frontend/src/main/task/task-data-service.ts` | Python `TaskDataClient` 通过 `/internal/task-data/*` 读取 / 提交，任务热路径不再回读 `DataManager` |
...
| 公开任务命令、任务回执与 task snapshot | `frontend/src/main/task` + `frontend/src/main/core/core-bridge-client.ts` | TS task service 校验公开请求并生成回执；Python Core 只通过 `/internal/runtime/tasks/*` 接收 Engine 命令 |
...
| 规则、提示词运行时读取 | `frontend/src/main/task/task-data-service.ts` | Python Engine 通过 `TaskDataClient` 获取任务所需 quality snapshot |
| 分析候选、checkpoint、分析结果 | `frontend/src/main/task/task-data-service.ts` + `frontend/src/main/database/` | Python Engine 通过 `TaskDataClient` 批量读取 / 提交，TS Gateway 统一推进持久事实 |
| 校对保存、校对 revision、重翻提交 | `frontend/src/main/service/proofreading-service.ts`、`frontend/src/main/task/task-data-service.ts` 与 `Engine/Retranslate/*` | 校对同步保存由 TS Gateway 调用 proofreading 服务直接写 `.lg`；重翻由 TS task service 校验 revision 后启动 Engine，批次提交经 `TaskDataClient` 回到 TS task data 服务 |
...
- 项目预过滤计算只在渲染层 runner / worker 中执行；create/open 草稿、打开对齐预演和事务化提交都由 TS 文件域 / 项目域提供，Python 任务侧通过 `TaskDataClient` 读取当前工程上下文。
...
```mermaid
flowchart TD
    A["TS TaskService"] --> B["/internal/runtime/tasks/*"]
    B --> C["Engine / Base.Event"]
    C --> D["TaskPipeline"]
    D --> E["TaskRequester"]
    D --> F["commit loop"]
    F --> G["TaskDataClient"]
    G --> H["/internal/task-data/*"]
    H --> I["TS TaskDataService"]
    I --> J["Electron main database service"]
```

稳定事实：
- `Engine` 负责执行骨架与 LLM 请求，任务数据读取和提交都经 `TaskDataClient` 回到 TS `TaskDataService`。
...
- 应用设置、最近项目由 TS main 的 `service/` 服务读写 `DATA_ROOT/userdata/config.json`，模型页 CRUD 由 TS main 的 `model/` 服务读写同一份配置；Python Core 的 `Config`、`ModelManager`、模型 `list-available/test` runner 与任务消费仍保留为内部运行时能力，但任务每次通过 `TaskDataClient` 读取当前上下文，不再依赖跨进程缓存刷新。
...
- 工作台文件写 mutation、项目设置对齐、translation reset、analysis reset、analysis glossary import、reset preview、项目轻生命周期、公开 bootstrap 运行态编码与 `project.patch` 补全由 TS main 的 `project/` 项目域承载，公开任务命令、task snapshot、任务数据内部路由与事件 hub 由 TS main 的 `task/` 任务域承载，校对同步保存仍由 TS main 的 `service/proofreading-service.ts` 承载；文件解析 / 写回由 TS main 的 `file/` 文件域承载；Python Core 保留 Engine 调度和任务命令入口，不再持有项目读侧缓存、项目同步或文件操作锁。
- 分析候选导入术语的预演和筛选属于前端 planner；Python 数据层保留候选聚合、候选数缓存和分析结果持久化。
- `translation reset` 与 `analysis reset` 属于同步 mutation，不是后台任务链路。
- 校对 `save-item`、`save-all`、`replace-all` 属于 TS main 承载的同步 mutation，写入后推进 items 与 proofreading revision；重翻通过 TS task service 的 `/api/tasks/start-retranslate` 进入任务型链路，`TaskRuntimeState` 持有公开忙碌态与 `retranslating_item_ids`，批次提交经 `TaskDataClient` 回到 TS 数据层并发布 `project.patch`。
...
### 引擎域
- TS `TaskRuntimeState` 是公开全局忙碌态的唯一权威来源；Python `Engine.status` 仍服务内部调度，但不再被公开 task snapshot 反查。
...
- 不要把共享任务语义写回 `module/Data`，也不要把工程事实塞进 `module/Engine`；Python Engine 需要工程事实时走 `TaskDataClient`。
