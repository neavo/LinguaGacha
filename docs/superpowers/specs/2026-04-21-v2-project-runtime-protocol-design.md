...
3. 迁移阶段允许 V1 与 V2 并存，但新增协议语义只能进入 `/api/v2/...`。
4. 当前仓库已经完成该迁移窗口：项目运行态协议边界只剩 `/api/v2/...` 与 `V2` / `v2` 文件路径，旧路由与旧 topic 已整体删除。
...
截至 2026-04-21，本阶段已经完成；当前仓库已删除内容包括：
...
当前实现已经满足以下退出条件，并据此完成 V1 项目运行态协议删除：
