...
- 主表格空态保持 `app-table` 表头与表格骨架，用占位空行表达，不在表格区域切换到独立 Empty 控件。
- 非表格列表若需要空提示，使用页面局部结构承载文案与样式，不恢复跨页面全局 Empty 抽象。
...
- 刷新失败不渲染页面内失败卡片；保留当前可用内容，并用 toast 告知。
- 表单输入非法使用红色边框；仅在提交、失焦或确认保存时弹出错误 toast。
- 搜索正则非法使用红色边框 + 警告图标 tooltip，不额外弹 toast。
- 其他常规接口、文件或任务错误默认使用 toast，不改变页面骨架。
