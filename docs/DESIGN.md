...
LinguaGacha 当前前端不是营销型网站，而是桌面端、高密度、任务驱动的工程工具。界面应当传达以下气质：
...
| 场景 | 建议字号 | 行高 | 说明 |
| --- | --- | --- | --- |
| 标题栏品牌、侧栏一级标签、卡片标题 | `14px` | `1.2` - `1.25` | 当前主流标题密度 |
...
- 表格是当前最强的信息密度载体，任何新增数据页面优先复用这一套视觉语言

### 6.6 命令栏

参考 `command-bar`：

- 高度固定，承担“当前页面能做什么”的明确表达
- 左侧操作组可换行，右侧提示信息弱化处理
...
- 提示、告警、确认框应沿用 shadcn 基础组件，但表面色和边框色继续服从当前主题变量
- 详情抽屉与确认弹窗优先强调任务语义和层级清晰，不做炫技式转场

## 7. 深度、边框与阴影

当前前端不是“纯平无层次”，而是用很轻的阴影和边界区分语义层：
...
> 为 LinguaGacha 生成界面时，应优先做“暖灰金色系、桌面工具化、高密度、低装饰、表格与面板优先”的 Electron 工作台，而不是营销站或插画化 SaaS 首页。
...
```text
请为 LinguaGacha 添加一个新的桌面工具页面。延续现有暖灰 + 金棕主色体系，保持 40px 标题栏、左侧折叠导航、右侧 page-shell 工作区。页面首屏直接进入可操作内容，不要做 hero，不要做营销卡片墙。优先使用卡片、表格、命令栏和设置行组织信息。
```

#### 新增设置模块

```text
请按 LinguaGacha 现有设置页风格实现该模块：14px 标题，12px 说明，单行单意图，右侧固定操作区，卡片圆角 4px，动作清晰，避免夸张装饰。
```

#### 新增数据页

```text
请复用 LinguaGacha 当前工作台语言：顶部少量关键统计，中间高密度表格，底部命令栏；hover、selected 与 active 通过浅底色和左侧细轨表达，不要使用厚描边或高饱和色块。
```
...
基于现有代码，LinguaGacha 的设计语言已经形成明确方向：

- 壳层稳定：透明标题栏 + 可折叠侧栏 + 带边框的工作区
- 主题稳定：暖灰中性表面 + 金棕强调色 + 亮暗双主题一致语义
- 组件稳定：设置卡片行、命令栏、表格、统计卡已形成统一风格
- 页面稳定：项目首页、工作台页、设置页分别覆盖启动、操作、配置三种核心场景

后续新增前端功能时，不需要重新寻找视觉方向，只需要继续沿着这条“桌面化工程工具”路线推进。
