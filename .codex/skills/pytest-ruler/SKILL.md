...
- 默认不要在测试里向控制台打印文本或留下调试输出
- 只有当输出本身就是被测行为，或者为了临时定位问题时才允许打印
- 如果需要验证输出，优先把输出当作显式断言对象，而不是靠肉眼看日志

## 测试文件组织

- 同一业务文件或同一被测对象的单元测试默认集中在一个主测试文件中，避免无理由分散
...
1. 先扫反模式：`__new__`、`call_args`、`call_args_list`、`tmp_path`、`mock_open`、`print\(`
