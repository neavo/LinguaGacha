---
name: project-docs
description: 用于整理 LinguaGacha 的长期项目文档，收口到 `docs/` 根目录并清除无效噪音。
---

# 项目文档

这份 skill 的目标不是“把旧文档润色得更像规范”，而是让 Agent 能把 LinguaGacha 的长期项目文档**整理、迁移并落地为固定的目标形态**。必要时可以整篇重写目标文档；**不要遵循最小化修改原则**，以最终结构正确、边界清楚、可持续维护为先。
...
1. 交付后的稳定项目文档必须集中到 `docs/` 根目录。
2. 交付后的 `docs/` 内只保留上面这 6 份长期文档，不增加并行总纲或额外长期文档。
3. 目录级遗留文档不属于交付后的长期文档形态；若其中仍承载稳定规则，默认动作是迁移到对应 `docs/*.md` 后删除遗留文档。
4. 交付后的 `AGENTS.md` 只保留 Agent 协作规则、仓库级硬约束、直接约束 Agent 编码行为的开发原则与技术栈风格硬约束、阅读起手式与交付要求，不承载专题文档主体内容。
5. 如果旧文档结构已经不适合交付形态，可以直接重写目标文档，不需要为了保留旧段落做补丁式迁移。
6. 除非用户明确要求，否则不要新增第 7 份、第 8 份长期文档。
...
| `docs/DESIGN.md` | 设计语言、视觉权威来源、页面骨架、组件语义是什么 | 具体接口协议、后端模块职责 |
...
| 有固定归宿 | 它能落进 `AGENTS.md` 或 6 份 `docs/*.md` 之一 | 迁移、合并或删除 |
...
3. 可以整篇重写目标文档，但不要把旧结构原样搬家。
4. 同一条稳定事实只保留一个权威版本，其他文档只做必要引用。
5. 如果本轮整理没有发生删除、迁移、合并或补写，只发生措辞替换，默认说明整理力度不足。
...
flowchart TD
    ...
    B --> C["把旧文档内容映射到 6 份 docs 与 AGENTS"]
    C --> D["重写目标文档"]
...
3. 建立迁移映射：把旧文档内容逐条映射到 6 份 `docs/*.md` 与 `AGENTS.md`
4. 直接重写目标文档：不必保留旧节名、旧段落顺序或旧目录划分
5. 迁移完成后删除失去价值的遗留目录级文档或并行文档
6. 更新 `AGENTS.md`，只保留协作入口、硬约束、编码硬约束、验证与交付要求
7. 自检所有链接、阅读路径、文档边界与规则一致性

## 遗留来源

下面这些对象如果出现在仓库里，只能作为迁移输入或反例理解，不属于交付后的长期文档：

- 目录级 `SPEC.md`
- 任何平行于固定 6 文档之外的 `docs/*.md`
- 承载任务过程、迁移过程、阶段性讨论的说明文档

其中，目录级 `SPEC.md` 在当前仓库里是**遗留迁移来源**，不是目标形态的一部分。
...
| 设计语言、视觉来源、页面骨架、稳定组件语义 | `docs/DESIGN.md` |
...
| 设计语言、视觉权威来源、页面骨架、组件语义 | `docs/DESIGN.md` |
...
- 整理完成后的长期文档是否真的收口为固定的 6 份 `docs/*.md` 加 `AGENTS.md`？
- 遗留目录级文档是否还残留稳定规则正文？
