...
|导出常量与手写字面量直接比较|测试只是在复述实现|删除；有独立契约时改测真实边界|
|多层测试重复同一预设或枚举列表|调整权威值造成连锁维护|权威层验证一次，下游只测消费行为|
|精确断言视觉参数、静态文案或展示数量|把内容调优误当程序契约|测关系、分支、占位符、可访问性或视觉证据|
|写死由另一个常量计算出的结果|上游调参导致无语义失败|从场景变量推导，或直接断言不变量|
...
print / console.log
真实 URL、用户目录或固定数据库地址
导出常量与 `toBe` / `toEqual` 中的同值字面量
同一数字、颜色、文案或枚举列表在实现与测试中的重复
```
