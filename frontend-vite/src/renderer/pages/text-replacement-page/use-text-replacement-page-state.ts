...
function build_statistics_badge_tooltip(
  ...
  const tooltip_lines = [
    t('text_replacement_page.statistics.hit_count').replace(
      '{COUNT}',
...
function move_selected_entries_by_direction(
  ...
  if (direction === 'top') {
    ...
    insert_index = next_entry_index < 0
...
export function useTextReplacementPageState(
  ...
  const [selection_anchor_entry_id, set_selection_anchor_entry_id] = useState<TextReplacementEntryId | null>(null)
  ...
  const [statistics_state, set_statistics_state] = useState<TextReplacementStatisticsState>(() => {
  ...
  const persist_merged_entries = useCallback(async (
    ...
    const { merged_entries, report } = merge_text_replacement_entries(
      entries,
      incoming_entries,
    )
  ...
  const open_edit_dialog = useCallback((entry_id: TextReplacementEntryId): void => {
  ...
  const toggle_regex_for_selected = useCallback(async (next_value: boolean): Promise<void> => {
    if (selected_entry_ids.length === 0) {
      return
    }
  ...
  const move_selected_entries = useCallback(async (
    direction: 'up' | 'down' | 'top' | 'bottom',
  ): Promise<void> => {
  ...
  const search_entry_relations_from_statistics = useCallback((entry_id: TextReplacementEntryId): void => {
    const target_index = entry_index_by_id.get(entry_id)
    const target_entry = target_index === undefined
      ? null
      : entries[target_index]
