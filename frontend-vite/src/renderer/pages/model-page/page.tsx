...
export function ModelPage(props: ModelPageProps): JSX.Element {
  ...
  const selector_model = model_page_state.snapshot.models.find((model) => model.id === model_page_state.selector_state.model_id) ?? null
  const confirm_is_destructive = model_page_state.confirm_state.kind === 'delete'
    ...
    <>
      <AlertDialog
        ...
        onOpenChange={(next_open) => {
          if (!next_open) {
            model_page_state.close_confirm()
          }
        }}
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              variant={confirm_is_destructive ? 'destructive' : 'default'}
              onClick={() => {
                void model_page_state.confirm_dialog()
              }}
            >
              {model_page_state.confirm_state.kind === 'delete'
                ? t('model_page.confirm.delete.confirm')
                : model_page_state.confirm_state.kind === 'reset'
                  ? t('model_page.confirm.reset.confirm')
                  : ''}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      ...
      <ModelBasicSettingsDialog
        ...
        onRequestTestModel={() => model_page_state.request_test_model(model_page_state.dialog_state.model_id ?? '')}
