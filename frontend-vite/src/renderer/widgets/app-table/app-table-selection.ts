...
export function normalize_app_table_selection_state(
  ...
  const active_row_id = state.active_row_id !== null && visible_row_id_set.has(state.active_row_id)
    ? state.active_row_id
    : null
  const anchor_row_id = state.anchor_row_id !== null && visible_row_id_set.has(state.anchor_row_id)
    ? state.anchor_row_id
    : null
...
export function collect_app_table_range_selection(
  ordered_row_ids: string[],
  anchor_row_id: string | null,
  target_row_id: string,
): string[] {
  const anchor_index = anchor_row_id === null
    ? -1
    : ordered_row_ids.indexOf(anchor_row_id)
...
export function build_app_table_click_selection_change(args: {
  ...
  if (args.range) {
    const anchor_row_id = args.current_state.anchor_row_id ?? args.current_state.active_row_id ?? args.target_row_id
    return {
