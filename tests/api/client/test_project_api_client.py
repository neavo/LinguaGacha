...
def test_project_api_client_reads_supported_extensions(
    fake_project_manager: FakeProjectManager,
    start_api_server: Callable[..., str],
) -> None:
    project_app_service = ProjectAppService(fake_project_manager)
    ...
    result = project_client.get_supported_extensions()

    assert set(result) == {".txt", ".json"}
