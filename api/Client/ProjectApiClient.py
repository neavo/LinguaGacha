...
class ProjectApiClient:
    ...
    def load_project(self, request: dict[str, Any]) -> ProjectSnapshot:
        ...
        response = self.api_client.post(V2ProjectRoutes.LOAD_PATH, request)
        return ProjectSnapshot.from_dict(response.get("project", {}))

    def create_project(self, request: dict[str, Any]) -> ProjectSnapshot:
        """创建工程并返回项目快照。"""

        response = self.api_client.post(V2ProjectRoutes.CREATE_PATH, request)
        return ProjectSnapshot.from_dict(response.get("project", {}))

    def get_project_snapshot(self) -> ProjectSnapshot:
        """查询工程快照，供 UI 首屏 hydration 使用。"""

        response = self.api_client.post(V2ProjectRoutes.SNAPSHOT_PATH, {})
        return ProjectSnapshot.from_dict(response.get("project", {}))

    def unload_project(self) -> ProjectSnapshot:
        """关闭当前工程并返回最新快照。"""

        response = self.api_client.post(V2ProjectRoutes.UNLOAD_PATH, {})
        return ProjectSnapshot.from_dict(response.get("project", {}))

    def get_supported_extensions(self) -> list[str]:
        """读取源文件选择器支持的扩展名。"""

        response = self.api_client.post(V2ProjectRoutes.EXTENSIONS_PATH, {})
    ...
    def collect_source_files(self, path: str) -> list[str]:
        ...
        response = self.api_client.post(V2ProjectRoutes.SOURCE_FILES_PATH, {"path": path})
    ...
    def get_project_preview(self, path: str) -> ProjectPreview:
        ...
        response = self.api_client.post(V2ProjectRoutes.PREVIEW_PATH, {"path": path})
