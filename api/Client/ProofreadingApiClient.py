...
class ProofreadingApiClient:
    ...
    def get_snapshot(
        ...
        response = self.api_client.post(V2ProjectRoutes.PROOFREADING_SNAPSHOT_PATH, request or {})
    ...
    def filter_items(self, request: dict[str, Any]) -> ProofreadingSnapshot:
        ...
        response = self.api_client.post(V2ProjectRoutes.PROOFREADING_FILTER_PATH, request)
        return ProofreadingSnapshot.from_dict(response.get("snapshot", {}))

    def search(self, request: dict[str, Any]) -> ProofreadingSearchResult:
        """执行校对页搜索，只返回命中信息。"""

        response = self.api_client.post(V2ProjectRoutes.PROOFREADING_SEARCH_PATH, request)
    ...
    def save_item(self, request: dict[str, Any]) -> ProofreadingMutationResult:
        ...
        response = self.api_client.post(V2ProjectRoutes.PROOFREADING_SAVE_ITEM_PATH, request)
    ...
    def save_all(self, request: dict[str, Any]) -> ProofreadingMutationResult:
        ...
        response = self.api_client.post(V2ProjectRoutes.PROOFREADING_SAVE_ALL_PATH, request)
        return ProofreadingMutationResult.from_dict(response.get("result", {}))

    def replace_all(self, request: dict[str, Any]) -> ProofreadingMutationResult:
        """执行批量替换，并返回写入结果。"""

        response = self.api_client.post(V2ProjectRoutes.PROOFREADING_REPLACE_ALL_PATH, request)
        return ProofreadingMutationResult.from_dict(response.get("result", {}))

    def recheck_item(self, request: dict[str, Any]) -> ProofreadingMutationResult:
        """重新检查单条条目，并返回警告结果。"""

        response = self.api_client.post(V2ProjectRoutes.PROOFREADING_RECHECK_ITEM_PATH, request)
        return ProofreadingMutationResult.from_dict(response.get("result", {}))
    ...
    def retranslate_items(self, request: dict[str, Any]) -> ProofreadingMutationResult:
        ...
        response = self.api_client.post(
            V2ProjectRoutes.PROOFREADING_RETRANSLATE_ITEMS_PATH,
            request,
        )
        return ProofreadingMutationResult.from_dict(response.get("result", {}))
