---
name: glossary-rules
description: 判断术语收录资格、实体身份、译名与字面覆盖，提供领域发现依据；工程规则提取、审校和维护与质量规则工作流组合使用。
---
...
语境确认多个形式确实属于同一业务对象或规则体系后，使用 `ws.tool.deriveCommonLiteralRoots` 枚举这些形式的公共连续字面片段。公共片段只作为词根候选，最终 `src` 由工程语境和完整匹配共同确定。

拟采用词根前，使用 `ws.tool.matchLiterals` 通过正式完整匹配比较：
...
1. 每个应收录对象至少由一条最终规则覆盖；
2. 词根合并前后保持全部已确认对象覆盖；
3. 每条最终规则的新增命中都必须分类为同一对象、别名或派生形式、清楚条件可处理的义项，或已收窄 / 拆分排除；
4. 每个无法判断的对象保持独立未决结论，并说明所缺事实和解除条件；
