...
|当前需要|证据用法|
|---|---|
|数量、零命中和分布|复用完整计数，或使用仅统计模式取得计数|
|候选资格、身份或译名事实|取得与具体问题相关的必要语境，保存实际判断范围及结论|
|覆盖差异、词根新增命中及边界分析|复用完整位置，或取得所需模式的完整证据，再完成语义判断|

覆盖计算需要完整的字段位置。先按工具契约检查证据是否收齐，再进行语义分类。证据不完整时，按当前检查的需要决定是否补取。

需要调查其它用法时，从已有证据或定点读取结果选择相关语境。多个对象需要上下文时，将对应 item ID 合批传给 `ws.tool.queryItemContexts`。按返回关系关联共享条目。只输出各问题所需字段。
