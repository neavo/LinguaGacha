...
采用候选前，使用 `ws.tool.matchLiterals` 在同一事实范围与匹配策略下取得完整匹配。按工具契约消费各模式的全部证据页，把完整字段命中记录留在工作资产中；快照或模式变化后重新取得受影响证据。由完整记录计算精确分支 item 并集 `B`、词根与分支 item 并集 `R`，逐项分类新增命中 `R - B`：

- 同一对象、别名或派生形式：取得共同身份及译法可组合的证据。
- 其它义项：确认局部翻译输入可观察条件，并按“表达路径与字段”形成条件说明。
...
- 每条选定规则的命中都满足身份、译名和边界要求；新增命中已安全分类，或经收窄、拆分排除。
