...
- **Body**（400，13px，1.5，0em）：用于常规内容、表格单元格、说明文字和控件正文；对话、校对上下文和结构化详情可把行高提高到 1.6–1.7，长段落控制在 65 到 75 个字符以内。
...
- **Markdown:** 正文使用 13px / 1.65；三级标题依次为 16px、14px、13px，代码、表格和 Mermaid 图使用 popover 面与 1px 边界承载。
- **Process Entries:** 工具、思考、上下文压缩与重试行最小高 34px、字号 12px；状态点直径 8px，并始终配合标签、耗时或错误文案。运行态允许 1s 脉冲，减少动态效果时改为静态光环。

### Agent Composer

- **Structure:** 输入器固定在对话工作面底部，外边距为 `0 16px 14px`，4px 圆角、1px 边界和 popover 背景；内部使用 `6px 6px 5px 12px` 紧凑内边距。
