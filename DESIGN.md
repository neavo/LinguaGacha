...
- **Process Entries:** 普通工具、思考、上下文压缩与失败恢复行保持紧凑、可扫描的状态语义。动态任务进度不作为普通工具进入历史信息流，只在输入器上方的紧凑状态条展示队首；进度标签与余项计数按内容占宽，队首获得状态条的剩余空间并在空间不足时截断，整条触发的 Tooltip 完整承载全部待办。运行态使用暖橙弧段圆环，减少动态效果时保留弧段但停止旋转。
