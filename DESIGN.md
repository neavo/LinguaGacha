...
LinguaGacha 的视觉系统要像一台可靠的原生桌面工具：固定标题栏、清楚侧栏、紧凑命令区、稳定表格和编辑器。萌感来自小尺寸图标、温和状态文案、暖橙选择线和轻微反馈；极客感来自单一等宽倾向字体、清晰数据密度和可重复操作路径。界面首先让用户相信批量翻译、术语、校对、文本保护和日志都能被追踪。

设计服务产品工作流。首屏进入任务、项目、设置、校对和文件处理，避免网页式浏览动线。`src/frontend/index.css` 是全局 token 和 shadcn 主题入口，`src/frontend/shadcn/` 提供基础控件底座，`src/frontend/widgets/` 承接表格、编辑器、命令栏、文件拖放等生产工作面。页面 CSS 只处理页面布局和局部组合状态。

**Key Characteristics:**
- 原生桌面客户端优先：固定壳层、侧栏、工作区边界和即时反馈。
- 冷灰浅色是默认长时间工作环境，暗色主题是低照度变体。
...
调色板是冷灰桌面壳层加低频暖橙。浅色主题承接长时间翻译和校对，暗色主题保持同一套层级关系，避免霓虹科技感、终端拟态和网页宣传感。

### Primary
- **图标暖橙主强调**：用于主按钮、当前导航轨、选中行轨、进度跳段、焦点环和关键可操作状态。它必须低频出现，像工具给出的线索。
- **暖米强调前景**：用于主强调色上的文字与图标，保持按钮和选中态清楚。

### Secondary
- **冷灰次级面**：用于次级按钮、分段切换、弱状态背景和工具栏分区。
- **石墨次级文字**：用于次级控件、说明文字和弱层级标签，弱于正文但保持可读。

### Tertiary
- **蜜黄图表色**：只用于图表、统计或进度分段辅助。
- **珊瑚图表色**：只用于分类或图表辅助，不能替代失败红。
- **钢灰图表色**：用于低优先级数据系列、背景统计和辅助参照。

### Neutral
- **冷灰背景**：应用主背景和工作区基底。
- **石墨前景**：正文、标题、图标默认色，避免纯黑带来的生硬网页感。
- **近白冷灰卡片面**：卡片、弹层、表格头、输入和编辑器承载面。
...
**The Status Honesty Rule.** 成功绿、警告橙、失败红只表达状态，禁止参与普通装饰配色。
...
**Character:** 字体系统带一点代码编辑器气质，让路径、模型名、术语、翻译条目、日志和表格数字显得可信。LGBaseFont 与系统中文字体负责中文可读性，避免营销页展示字体路线。

### Hierarchy
- **Display**（400，42px，1）：只用于统计数字、关键计数和大号任务状态值。
- **Headline**（500，16px，1.3）：用于页面重要分组标题、弹窗标题和覆盖层标题。
...
**The Zero Drama Spacing Rule.** 新增可见文本的 `letter-spacing` 默认使用 `0em`；编辑器特殊字符可视化和表格序号等生产理由才允许局部偏离。

## 4. Elevation

LinguaGacha 使用低阴影加描边的混合层级。常驻结构靠背景色、1px 边框、分割线和壳层分区表达；阴影只给卡片、表格头、命令栏、拖拽浮层和覆盖层提供轻微浮起。文件拖放覆盖层可使用淡渐变和 4px 模糊，因为它是临时状态反馈。
...
### Buttons
- **Shape:** shadcn 按钮默认 4px 半径，基础高度 32px；标题栏按钮为 32px，工具栏按钮为 36px。
- **Primary:** 暖橙背景、暖米文字，高度 32px，水平内边距 10px，图标由组件按 `data-icon` 控制尺寸。
- **Hover / Focus:** hover 轻微加深或转为 muted 背景；focus 使用 ring 边框和 3px 半透明焦点环；active 允许 1px 下压。
...
- **State:** 选中、筛选、状态徽标必须辅以文字、图标或明确语义，禁止只依赖颜色。

### Cards / Containers
- **Corner Style:** 卡片默认 4px，表格、工具栏、设置项都沿用小半径；工作面禁止大圆角网页卡片气质。
- **Background:** default、panel、table、toolbar 四种变体由 `src/frontend/index.css` 的 `data-slot="card"` 规则统一控制。
...
### Inputs / Fields
- **Style:** 输入框高 32px，4px 到 6px 视觉半径，透明背景，1px input 边框，水平内边距 10px。
- **Focus:** focus-visible 使用 ring 边框；编辑器聚焦时切换到 popover 背景，反馈清楚但克制。
...
### Tables
- **Structure:** 表头 42px，行高 36px 到 39px，单元格水平内边距 12px，列分割线只用 1px 柔灰。
...
- **Readonly / Invalid:** 只读态降低前景和背景对比；无效态使用 failure 色，但必须保留文本可读性。
...
- **Limit:** 这个处理只属于拖放状态，禁止扩散成常驻玻璃拟态背景。

## 6. Do's and Don'ts

### Do:
- **Do** 把视觉权威放在 `DESIGN.md`，把代码 token 权威放在 `src/frontend/index.css`。
- **Do** 让 `src/frontend/shadcn/` 维护基础控件视觉，让 `src/frontend/widgets/` 维护生产工作面组件。
...
### Don't:
- **Don't** 制造“网页感”：整体视觉、布局节奏和交互反馈禁止像浏览器网页或 Web SaaS 套壳。
...
- **Don't** 用超过 1px 的侧边彩条装饰卡片、列表项、提示或警告；3px 选择轨只用于导航、表格选中和明确交互状态。
- **Don't** 在新文档、脚本提示或样式说明里继续引用旧 `src/renderer/` 目录作为当前事实。
