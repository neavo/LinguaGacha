...
- **List Entry:** 点击技能条目进入详情。开关与拖动把手独立操作，悬停和键盘焦点沿用卡片反馈。
- **Frame:** 顶栏、文件树和文档使用三张卡片，间距 8px。顶栏高 40px，路径文字统一，分隔符弱化。文件栏默认宽 196px，间隙中的手柄支持拖动和键盘调整。
- **Document:** 名称、描述和正文共用一个连续编辑器、行号栏与滚动区域。头部使用 `name:`、`description:` 和 `---` 呈现，字段名使用主题色，分隔符弱化，字段值沿用编辑器文字色。长描述自动折行，无效字段使用波浪下划线并显示说明。普通文件占满文档卡片，内置技能沿用公共只读样式。
- **Navigation:** 文件操作在悬停、聚焦和菜单展开时显示，触摸设备保持可见。加载与错误使用公共内容状态组件。
...
- **Process Entries:** 普通工具、思考、上下文压缩与失败恢复行保持紧凑、可扫描的状态语义。Todo 只在输入器上方的紧凑状态条展示队首；标签与余项计数按内容占宽，队首获得状态条的剩余空间并在空间不足时截断，整条触发的 Tooltip 完整承载全部待办。运行状态点使用主题橙，翻译摘要运行时使用成功绿、停止过程中使用警告橙。状态点与决定按钮共享 1 秒 ease-out 扩散光环：从主体边缘向外扩散 6px，在周期 70% 时完全淡出；状态点主体为 8px，决定按钮主体为 24px。减少动态效果时，状态点保留 3px 静态光环。
- **Message Actions / Media:** 可修订消息的复制与编辑位于消息旁的紧凑操作区，编辑在原消息位置展开，失败信息留在编辑器旁。图片与 Mermaid 共用 `xl` 媒体预览，支持缩放、拖动和重置视图；按钮与键盘操作共同承接查看过程。

- **Translation Status:** Agent 卡片与工作台胶囊共用翻译摘要：中性边框和文字、区分翻译中（绿色）与停止中（橙色）的状态灯、浅主题色工程进度填充。状态与速度以中点分隔，窄宽度优先省略速度；卡片文字居中，胶囊按内容收紧，省略百分比。进度与状态灯遵循减少动态效果偏好。两者点击打开详情；工作台在任务开始时自动提示，Agent 在终态恢复 Todo。

- **Round Status:** 每回合状态条以中点连接状态、耗时和 TPS。运行时使用状态灯，结束后使用分隔线。速度使用灰色小字、两位小数和等宽数字，状态条宽度不超过 360px 时隐藏 TPS，优先保留状态和耗时。

### Agent Composer

- **Structure:** 输入器与动态待办状态条共用对话工作面底部组合区，操作区最大宽 1120px，状态条空时不占位；状态行高 28px，输入器使用 4px 圆角、1px 边界、popover 背景与紧凑内边距，操作表面最小高 106px。
- **Editor:** CodeMirror 输入区最小高 64px、最大高 140px、13px / 1.5，超出后内部滚动。页脚与独立操作统一高 28px，发送、停止与跟随最新使用圆形，发送与换行快捷键由发送按钮 Tooltip 渐进展示，队列行内操作高 24px，承接附件、合并的模型及思考等级菜单、会话用量 Tooltip 和发送操作。已选模型的等级显示在入口，批量翻译“保持一致”只显示原文案。
