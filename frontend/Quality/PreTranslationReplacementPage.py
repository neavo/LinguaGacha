...
class PreTranslationReplacementPage(QWidget, Base):
    ...
    PRESET_PATH: str = "resource/pre_translation_replacement_preset"
    ...
    def __init__(self, text: str, window: FluentWindow) -> None:
        ...
        if Localizer.get_app_language() == BaseLanguage.ZH:
            self.default = {
                ...
                "pre_translation_replacement_regex": False,
    ...
    def add_command_bar_action_preset(self, parent: CommandBarCard, config: dict, window: FluentWindow) -> None:
        ...
        def reset() -> None:
            ...
            config = self.save_config(config)
            TableHelper.update_to_table(self.table, config.get("pre_translation_replacement_data"), __class__.KEYS)
        ...
        def apply_preset(filename: str) -> None:
            ...
            # 向表格更新数据
            TableHelper.update_to_table(self.table, config["pre_translation_replacement_data"], __class__.KEYS)

            # 从表格加载数据（去重后）
            config["pre_translation_replacement_data"] = TableHelper.load_from_table(self.table, __class__.KEYS)
    ...
    def add_command_bar_action_regex(self, parent: CommandBarCard, config: dict, window: FluentWindow) -> None:
        ...
        def update_ui(config: dict) -> None:
            if config.get("pre_translation_replacement_regex") == True:
        ...
        def triggered() -> None:
            config = self.load_config()
            config["pre_translation_replacement_regex"] = config.get("pre_translation_replacement_regex") == False
            config = self.save_config(config)
            update_ui(config)
