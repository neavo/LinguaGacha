...
class CustomPromptZHPage(QWidget, Base):
    ...
    PRESET_PATH: str = "resource/custom_prompt/zh"
    ...
    def __init__(self, text: str, window: FluentWindow) -> None:
        ...
        # 载入并保存默认配置
        config = Config().load()
        if config.custom_prompt_zh_data == None:
            config.custom_prompt_zh_data = PromptBuilder(config).get_base(BaseLanguage.Enum.ZH)
        config.save()
    ...
    def add_widget_body(self, parent: QLayout, config: Config, window: FluentWindow) -> None:
        self.prefix_body = EmptyCard("", PromptBuilder(config).get_prefix(BaseLanguage.Enum.ZH))
        self.prefix_body.remove_title()
        parent.addWidget(self.prefix_body)

        self.main_text = PlainTextEdit(self)
        self.main_text.setPlainText(config.custom_prompt_zh_data)
        parent.addWidget(self.main_text)

        self.suffix_body = EmptyCard("", PromptBuilder(config).get_suffix(BaseLanguage.Enum.ZH).replace("\n", ""))
    ...
    def add_command_bar_action_preset(self, parent: CommandBarCard, config: Config, window: FluentWindow) -> None:
        ...
        def apply_preset(filename: str) -> None:
            ...
            # 更新配置文件
            config = Config().load()
            config.custom_prompt_zh_data = prompt
            config.save()
