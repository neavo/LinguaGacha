...
export function configure_renderer_public_path(desktop_bundle_dir: string): void {
  process.env.VITE_PUBLIC = is_development_mode()
    ? path.join(resolve_frontend_bundle_root(desktop_bundle_dir), "public")
...
/**
 * 前端 bundle 根只用于定位 dist/public，不承载应用 APP_ROOT 语义。
 */
function resolve_frontend_bundle_root(desktop_bundle_dir: string): string {
  return path.join(desktop_bundle_dir, "..");
}

/**
 * 发布态固定加载 renderer dist，开发态由 dev server 接管页面入口。
 */
function resolve_renderer_dist(desktop_bundle_dir: string): string {
  return path.join(resolve_frontend_bundle_root(desktop_bundle_dir), "dist");
}
