...
import { NPM_INITIAL_CWD_ENV_NAME, resolve_core_app_root } from "./lifecycle-command-resolver";
...
describe("resolve_core_app_root", () => {
  it("优先从 INIT_CWD 向上查找包含 resource 与 version.txt 的应用根", () => {
    ...
    const nested_dir = path.join(app_root, "frontend", "src");
  ...
  it("找不到应用根标记时返回解析后的 appRoot", () => {
    const app_root = fs.mkdtempSync(path.join(os.tmpdir(), "linguagacha-root-fallback-"));
