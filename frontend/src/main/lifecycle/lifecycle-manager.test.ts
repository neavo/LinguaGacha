...
import { build_core_process_env, build_core_process_spawn_request } from "./lifecycle-manager";
import type { CoreLaunchCommand } from "./lifecycle-types";
...
describe("build_core_process_spawn_request", () => {
  ...
  it("使用 core.exe 启动目标并注入生命周期环境变量", () => {
    ...
    expect(request.options.env["LINGUAGACHA_CORE_API_TOKEN"]).toBe(instance_token);
  });
