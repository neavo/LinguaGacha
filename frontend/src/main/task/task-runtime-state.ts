...
// 这些终态不会再占用全局任务锁，TS busy 权威必须在入站事件处即时释放。
const IDLE_STATUSES = new Set(["DONE", "ERROR", "IDLE"]);

/**
 * TS Gateway 内的任务运行态权威，替代旧 Python `/internal/runtime/tasks/state` 查询。
 */
export class TaskRuntimeState {
  ...
  /**
   * 任务命令被 TS 受理后立即占用运行态，避免按钮等到下一帧 SSE 才变化。
   */
  ...
  /**
   * project.patch 里的 replace_task 是任务块最终口径，需要回灌到 TS 运行态。
   */
