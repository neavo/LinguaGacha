...
/**
 * 分析链路控制码伪名注入器，避免模型把控制码误识别成普通术语。
 */
export class AnalysisFakeNameInjector {
  ...
  public restore_glossary_entry(src: string, dst: string): [string, string] | null {
    ...
    if (!AnalysisFakeNameInjector.is_control_code_text(restored_src)) {
      return null;
    }
  ...
  public static is_control_code_self_mapping(src: string, dst: string): boolean {
    ...
    return (
      ...
      AnalysisFakeNameInjector.is_control_code_text(normalized_src)
    );
  }
