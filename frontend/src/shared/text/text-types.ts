...
/**
 * 文本处理只依赖的配置字段，字段名保持 `Config.py` 快照兼容。
 */
export interface TextProcessingConfig {
  // 源/目标语言直接来自项目配置，语言过滤器负责未知值兜底。
  source_language: string;
  target_language: string;
  // clean_ruby 控制是否剥离注音标记，保持与导入格式处理器解耦。
  clean_ruby: boolean;
  // 残留和相似度检查按开关独立启用，便于旧项目逐步迁移质量规则。
  check_kana_residue: boolean;
  check_hangeul_residue: boolean;
  check_similarity: boolean;
  // 自动保护前后缀开关决定完全保护行是否仍进入翻译流程。
...
export interface TextQualitySnapshot {
  // glossary 与 replacement 规则均为运行时快照，worker 不回读数据库。
  ...
  // prompt 字段来自提示词设置页，PromptBuilder 负责与资源模板合并。
...
export type TextTaskItemRecord = TextJsonRecord & {
  // id/item_id 同时兼容数据库行和前端运行态条目。
  id?: number;
  item_id?: number;
  // src/dst/status 是翻译提交的核心事实，其余字段只辅助处理。
  src?: string;
  dst?: string;
  name_src?: string | string[] | null;
  name_dst?: string | string[] | null;
  status?: string;
  // text_type 决定保护规则分支，retry_count 用于任务调度诊断。
  text_type?: string;
  retry_count?: number;
  // extra_field 保留格式处理器回写所需的结构化上下文。
  extra_field?: TextJsonValue;
};
...
export class TextProcessingConfigTool {
  /**
   * 从完整 config 快照抽取文本处理配置，缺失时使用 `Config.py` 默认值。
   */
