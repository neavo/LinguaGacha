...
function build_highlight_decorations(
  highlights: ProofreadingCodeEditorHighlight[],
): DecorationSet {
...
const light_editor_theme = create_editor_theme(light_editor_palette, false)
const dark_editor_theme = create_editor_theme(dark_editor_palette, true)
...
function resolve_hovered_highlights(
  ...
  const shortest_length = Math.min(...containing_highlights.map((highlight) => {
...
function create_editor_extensions(
  ...
  return [
    ...
    hoverTooltip((view, pos, side): Tooltip | null => {
      void view
      const hovered_highlights = resolve_hovered_highlights(
        highlights_ref.current,
        pos,
        side,
      )
      ...
      const tooltip_lines = [...new Set(hovered_highlights.map((highlight) => {
...
export function ProofreadingCodeEditor(
  props: ProofreadingCodeEditorProps,
): JSX.Element {
  ...
  useEffect(() => {
    ...
    editor_view.dispatch({
      effects: [
        editor_readonly_compartment.reconfigure(EditorState.readOnly.of(props.read_only)),
      ],
    })
  }, [props.read_only])
