  ...
  if (text_lines.length === 1) {
    ...
    return text_lines.flatMap((line, line_index) => {
      ...
      if (is_last_line) {
        ...
        return [
          line,
          createElement('br', { key: `${key_prefix}-line-break-${line_index}` }),
        ]
      }
    })
  }
...
function render_rich_text_node(
  ...
  if (child_node.nodeType === TEXT_NODE_TYPE) {
    ...
    const element_children = render_rich_text_nodes(element_node.childNodes, component_map, `${key_prefix}-child`)
    ...
    if (component_renderer !== undefined) {
      return createElement(
        Fragment,
        { key: key_prefix },
        component_renderer(element_children),
      )
    } else {
...
function parse_rich_text_root(source_text: string): Element | null {
  if (typeof DOMParser === 'undefined') {
    ...
    const document = new DOMParser().parseFromString(`<lg-rich-text>${source_text}</lg-rich-text>`, 'text/html')
...
export function render_rich_text(source_text: string, component_map: RichTextComponentMap): ReactNode {
