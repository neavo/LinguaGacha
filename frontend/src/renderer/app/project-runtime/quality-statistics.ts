...
function count_rule_occurrences(
  ...
  const flags = rule.mode === 'text_preserve'
    ? 'iu'
    : rule.case_sensitive
      ? 'u'
      : 'iu'
  const compiled_pattern = compile_pattern(
    is_regex_mode ? pattern : escape_regexp(pattern),
    flags,
  )
...
function build_subset_relation_map(args: {
  ...
  const scope_snapshots = dedupe_relation_scope(
    build_relation_snapshots(args.relationCandidates),
  )
...
export function collect_project_item_texts(
