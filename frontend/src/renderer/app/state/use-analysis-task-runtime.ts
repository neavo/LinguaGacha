...
function resolve_analysis_terminal_feedback_message(args: {
  ...
  if (!is_active_analysis_task_status(args.previous_status) || args.previous_status === 'STOPPING') {
    return null
  }
...
export function useAnalysisTaskRuntime(
  ...
  const [last_analysis_task_snapshot, set_last_analysis_task_snapshot] = useState<AnalysisTaskSnapshot | null>(null)
  ...
  const [analysis_confirm_state, set_analysis_confirm_state] = useState<AnalysisTaskConfirmState | null>(null)
  ...
  const analysis_task_menu_busy = analysis_importing || (
    analysis_confirm_state !== null
    && analysis_confirm_state.submitting
  )
  const analysis_task_menu_disabled = !project_snapshot.loaded
    || task_snapshot.busy
    || analysis_task_menu_busy
  const can_open_analysis_detail_sheet = project_snapshot.loaded
    && analysis_task_display_snapshot !== null
  const analysis_task_active = is_active_analysis_task_status(
  ...
  const should_animate_analysis_waveform = analysis_task_active
    || has_unsettled_analysis_waveform_tail
  ...
  const append_analysis_waveform_sample = useEffectEvent((): void => {
    ...
    const next_visual_snapshot = analysis_task_display_snapshot === null
      ? null
    ...
    if (analysis_task_active) {
      const has_progress_delta = previous_observed_snapshot !== null
  ...
  const clear_analysis_task_state = useCallback((): void => {
    ...
    set_analysis_task_metrics(resolve_analysis_task_metrics({
      snapshot: null,
      now_seconds: 0,
    }))
  ...
  }, [apply_analysis_task_snapshot, clear_analysis_task_state, project_snapshot.loaded, push_toast, t])
  ...
  useEffect(() => {
    ...
    apply_analysis_task_snapshot(normalize_analysis_task_snapshot_payload({
      task: task_snapshot,
    }))
