...
export const WORKBENCH_WAVEFORM_MAX_POINTS = 256

const WORKBENCH_WAVEFORM_IDLE_DECAY_RATIO = 0.78
const WORKBENCH_WAVEFORM_ZERO_THRESHOLD = 1

export function append_workbench_waveform_sample(
  history: number[],
  sample: number,
): number[] {
...
export function has_unsettled_workbench_waveform_tail(history: number[]): boolean {
  const visible_history = history.length <= WORKBENCH_WAVEFORM_VISIBLE_POINTS
    ? history
