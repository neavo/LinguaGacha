...
describe("ProjectChangeEventAdapter", () => {
  ...
  function create_data_reader(options: {
    ...
    return {
      ...
      get_runtime_section_revision: (_meta: ProjectDataJsonRecord, section: string) =>
...
describe("ProjectChangeCoordinator", () => {
  it("用同一 meta 快照校验 revision 并生成运行态 bump 操作", () => {
    ...
    const coordinator = new ProjectChangeCoordinator(database, null, new ProjectEventBus());
  ...
  it("统一提交方法在 revision 冲突时不构造事务且不发布事件", async () => {
    ...
    const coordinator = new ProjectChangeCoordinator(
      database,
