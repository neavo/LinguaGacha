...
// ProjectEventType 是 Core 内部 committed event 词表，和公开 SSE topic 分离。
...
// ProjectEvent 是 Core 内部事件总线唯一事件联合类型。
...
// ProjectEventHandler 是 Core 内部 committed event 的订阅入口，按事件类型收窄 payload。
...
// ProjectEventBus 是 Core 内部事务提交后事件分发器，不直接承担公开 SSE 或 renderer 刷新策略。
