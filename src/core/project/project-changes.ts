...
import type { ProjectEvent, ProjectEventBus } from "./project-events";
...
export class ProjectChangeEventAdapter {
  ...
  private readonly data_reader: ProjectDataReader; // DB -> API payload 投影集中在无状态服务
  ...
  private build_section_revision_payload(
    ...
    for (const section of updated_sections) {
      section_revisions[section] = this.data_reader.get_runtime_section_revision(meta, section);
    }
...
export class ProjectChangeCoordinator {
  ...
  private readonly project_event_bus: ProjectEventBus; // 内部 committed event 先于公开变更发布，供 Core cache 维护热数据
  ...
  public async publish_app_events_for_committed_change(
    ...
    for (const event of this.build_app_events_after_commit(request)) {
      await this.project_event_bus.publish(event);
    }
  }
   ...
   */
  private build_app_events_after_commit(request: ProjectMutationChangeRequest): ProjectEvent[] {
