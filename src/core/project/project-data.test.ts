...
import { build_section_revisions_from_meta, get_runtime_section_revision } from "./project-data";
...
describe("ProjectDataCache", () => {
  it("热机后缓存当前工程 items、质量块、提示词块和 section revision", async () => {
    const cache = new ProjectDataCache(
      create_database({
        meta: {
  ...
  it("unload 事件只清理当前工程缓存", async () => {
    const cache = new ProjectDataCache(create_database({ items: [create_item()] }));
    await cache.warmProject("E:/Project/demo.lg");
  ...
  it("已提交 mutation 后缓存维护失败会进入可恢复状态，后续读取重新热机", async () => {
    ...
    const cache = new ProjectDataCache(database, log_manager);
    await cache.warmProject("E:/Project/demo.lg");
