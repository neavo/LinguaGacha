...
import type { ProjectReadModelWorkerPool } from "./worker/project-read-model-worker-pool";
...
export class NameFieldExtractionReadModelService {
  ...
  public async read(request: Record<string, ApiJsonValue>): Promise<MutableJsonRecord> {
    ...
    const glossary_entries = Array.isArray(glossary["entries"])
      ...
      : [];
    const result = await this.worker_pool.extract_name_fields(
      {
