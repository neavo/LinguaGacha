...
describe("core-bundle-contract", () => {
  ...
  it("把 worker_threads 执行入口固定到桌面 bundle 根目录", () => {
    const bundle_root = path.join(process.cwd(), "build", "dist-electron");
    const execution = build_worker_threads_execution_from_desktop_bundle_dir(bundle_root);

    expect(execution).toEqual({
      kind: "worker_threads",
      workerEntryUrl: pathToFileURL(path.join(bundle_root, CORE_WORKER_ENTRY_FILE_NAME)),
    });
  });
});
