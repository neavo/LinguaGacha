...
/**
 * Core 内部 .lg 物理读写入口，集中持有 SQLite、事务和 asset 压缩格式
 */
export class ProjectDatabase {
  ...
  public read_asset_content(project_path: string, asset_path: string): Buffer | null {
    return this.with_project_connection(project_path, (db) => {
      const row = db.prepare("SELECT data FROM assets WHERE path = ?").get(asset_path); // 调用方只消费解压后的原始 bytes，Zstd 格式细节留在 Core 内部
      if (row === undefined) {
        return null;
      }
