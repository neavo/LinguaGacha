...
import { get_runtime_section_revision } from "../project/project-data";
...
function create_service(
  ...
  const project_operation_gate = new ProjectOperationGate(task_runtime_state);
  return {
    database,
...
/**
 * 暂停下一次格式解析，稳定复现慢准备阶段持有结构性 mutation lease 的窗口
 */
...
describe("WorkbenchService", () => {
  ...
  it("translation reset all 解析窗口内拒绝另一段结构性 mutation", async () => {
    const { database, service, lg_path } = create_service();
    const source_path = project_path("a.txt");
    fs.writeFileSync(source_path, "新", "utf-8");
    database.execute({
