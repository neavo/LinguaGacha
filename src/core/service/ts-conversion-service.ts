...
import type { ProjectReadModelWorkerPool } from "../project/read-model/worker/project-read-model-worker-pool";
import type { FileExportService } from "../file/file-export-service";
...
import type { QualityRulePresetReader } from "./quality-rule-preset-reader";

type JsonRecord = Record<string, ApiJsonValue>;

export class TsConversionService {
  private readonly session_state: ProjectSessionState;
  private readonly app_session_cache: AppSessionCache;
  private readonly worker_pool: ProjectReadModelWorkerPool;
  ...
  public constructor(options: {
    ...
    workerPool: ProjectReadModelWorkerPool;
    ...
    this.worker_pool = options.workerPool;
  ...
  public async export_converted_translation(request: JsonRecord): Promise<JsonRecord> {
    ...
    const source_items = this.app_session_cache.readItems();
    if (source_items.length === 0) {
      throw new AppErrors.RequestValidationError({
        diagnostic_context: { reason: "empty_ts_conversion_items" },
      });
    }
    const runtime_items = normalize_ts_conversion_runtime_items(source_items);
    ...
    const preset_rules_by_text_type =
      ...
      normalized_text_preserve_mode !== "custom"
        ...
        : {};
    const converted_items = await this.worker_pool.convert_ts_items(
      {
        items: runtime_items,
        direction,
        convert_name,
