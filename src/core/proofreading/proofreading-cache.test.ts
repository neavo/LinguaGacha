...
function create_worker(): CoreWorkerClient & {
  hydration_inputs: ProofreadingRuntimeHydrationInput[];
} {
  const hydration_inputs: ProofreadingRuntimeHydrationInput[] = [];
  return {
    hydration_inputs,
    run: vi.fn(async (task: { type: string; input: ProofreadingRuntimeHydrationInput }) => {
      if (task.type !== "proofreading_hydration") {
        throw new Error(`测试未实现 task：${task.type}`);
      }
      hydration_inputs.push(task.input);
      return evaluateProofreadingRuntimeSlice(task.input);
    }),
    dispose: vi.fn(async () => undefined),
  } as unknown as CoreWorkerClient & {
    hydration_inputs: ProofreadingRuntimeHydrationInput[];
  };
}
