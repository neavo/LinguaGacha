...
export class ApiGatewayServer {
  ...
  private create_app(): Hono {
    ...
    const app_setting_service = services.app_setting_service;
      ...
      try {
        ...
        if (status >= 500 || normalized_error.severity !== "expected") {
          record_app_error(normalized_error, {
            logManager: this.options.coreServices.log_manager,
  ...
  private require_loaded_project_path(): string {
    const state = this.options.coreServices.project_session_state.snapshot();
  ...
  private error_to_envelope(error: AppError, request_id: string) {
    return api_error(
      to_api_error_payload(error, request_id, this.options.coreServices.resolve_api_text()),
    );
  }
  ...
  private create_log_stream_response(): Response {
    ...
    const stream = new ReadableStream<Uint8Array>({
      start: (controller) => {
        ...
        unsubscribe = this.options.coreServices.log_manager.subscribe((event) => {
