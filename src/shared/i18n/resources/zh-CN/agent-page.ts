export const zh_cn_agent_page = {
  title: "AGENT",
  thinking: "思考过程",
  thinking_active: "正在思考",
  diagram: {
    open_preview: "放大预览图表",
    preview_title: "图表预览",
    rendering: "正在生成图表 …",
    render_failed: "图表渲染失败，已显示 Mermaid 源码。",
  },
  image: { title: "图片", open_preview: "打开图片预览" },
  media: {
    canvas_label: "媒体预览画布",
    controls_label: "媒体缩放控制",
    zoom_out: "缩小",
    zoom_in: "放大",
    reset_zoom: "重置缩放",
  },
  loading: "正在恢复会话 …",
  empty: {
    message: "「搭档」，我们接下来做点什么呢  ( •̀ ᗜ •́ )つ▱",
    suggestions: {
      capabilities: "介绍你的能力",
      roleplay: "开始角色扮演",
      quality_rule_workflow: "帮我提取术语",
      translation_workflow: "帮我审校译文",
    },
  },
  input: {
    placeholder: "描述任务，或输入 @ 选择技能或术语 …",
    edit_assistant_placeholder: "修改模型回复 …",
    newline: "换行",
    drop_images: "松开以添加图片",
  },
  mention: {
    groups: { skills: "技能", terms: "术语" },
    no_matches: "没有匹配的项目 …",
    term_hits: "{count} 次",
  },
  annotation: {
    title: "批注",
    add: "添加批注",
    edit: "修改批注",
    remove: "删除",
    selected_text: "文本",
    user_comment: "批注",
    comment_placeholder: "写下针对所选文本的评论 …",
  },
  context_usage_warning: "即将自动压缩上下文",
  approval: {
    manual: "手动批准",
    auto: "自动批准",
    tooltip_manual: "写入请求 - 手动批准",
    tooltip_auto: "写入请求 - 自动批准",
    description: "正在写入工程数据：{summary}",
    summary_items: "文本数据 {count} 条",
    summary_glossary: "术语规则 {count} 条",
    summary_text_preserve: "文本保护规则 {count} 条",
    summary_pre_replacement: "译前替换规则 {count} 条",
    summary_post_replacement: "译后替换规则 {count} 条",
    summary_prompts: "自定义提示词 {count} 条",
    summary_separator: "，",
    summary_last_separator: "，",
    reject: "拒绝",
    approve_once: "允许一次",
    approve_future: "本任务后续全部允许",
  },
  compaction: {
    running: "正在压缩上下文 …",
    success: "上下文压缩成功 …",
    error: "上下文压缩失败 …",
  },
  action: {
    send: "发送",
    stop: "停止",
    applying: "正在应用工程修改，完成前不可停止",
    new_task: "新任务",
    add_image: "添加图片",
    continue: "继续",
    edit: "修改",
    copy: "复制",
    copied: "已复制",
    copy_failed: "复制失败",
    follow_latest: "跟随最新",
  },
  editing: { user: "正在修改你的消息", assistant: "正在修改模型回复", queue: "正在修改排队消息" },
  queue: {
    no_message_text: "无消息正文",
    attachments: "{count} 个附件",
    reorder: "拖动排序",
    send_now: "立即发送",
    sending: "正在发送",
    delete: "删除消息",
    full: "消息队列已满 · {count}/{limit}",
  },
  confirm: {
    new_task: "是否确认开始新的对话任务 …?",
    thinking_off: "模型思考关闭时 AGENT 任务智能会显著下降，是否确认继续执行 …?",
  },
  status: { running: "正在处理", success: "已完成", error: "失败", stopped: "已停止" },
  task_progress: { pending: "待办事项" },
  tool: {
    details: "{tool} 工具调用详情",
    input: "输入",
    output: "输出",
  },
  round: {
    running: "处理中 {duration}",
    success: "已完成 · {duration}",
    error: "失败 · {duration}",
    stopped: "已停止 · {duration}",
  },
  error: {
    restore: "会话恢复失败，请重试。",
    connection: "连接中断，正在等待重连。",
    send: "发送失败，草稿已保留。",
    continue: "继续失败，请再次尝试。",
    edit: "消息修改失败，编辑内容已保留。",
    queue_update: "排队消息修改失败，编辑内容已保留。",
    queue_delete: "排队消息删除失败，请重试。",
    queue_reorder: "队列顺序更新失败，请重试。",
    queue_send: "消息无法立即发送，仍保留在队列中。",
    image: "图片读取失败，仅支持 PNG、JPG、BMP、WebP 和 AVIF。",
    stop: "停止失败，请重试。",
    reset: "新任务创建失败，请重试。",
    approval_mode: "写入请求审批模式更新失败，请重试。",
    approval_decision: "写入请求处理失败，请重试。",
    terms_load: "术语加载失败，技能选择与消息发送仍可使用。",
  },
  unavailable: {
    restoring: "正在恢复会话",
    runtime_busy: "其它任务正在运行",
    settling: "正在结束当前任务",
  },
} as const;
