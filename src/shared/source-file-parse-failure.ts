import { app_error_message_key, is_app_error_code, type AppErrorCode } from "./error/app-error";
...
export type SourceFileParseFailureRecord = {
  ...
  code: AppErrorCode; // 前端和日志按稳定错误码推导当前语言下的可见原因
};
...
export function format_source_file_parse_failure_notice(args: {
  ...
  return args.failures
    .map((failure) => {
      return `${failure.filename} - ${args.text(app_error_message_key(failure.code))}`;
    })
    .join("\n");
}
...
function normalize_source_file_parse_failure(value: unknown): SourceFileParseFailureRecord | null {
  ...
  if (filename === "" || !is_app_error_code(code)) {
    return null;
  }
  return {
    source_path,
