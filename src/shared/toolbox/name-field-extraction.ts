...
export type NameFieldGlossaryEntry = {
  src: string;
  dst: string;
  info: string;
...
function normalize_reader_item_record(value: unknown): ItemRecord | null {
...
export function extract_name_field_rows(args: {
  ...
  for (const value of args.items) {
    const item = normalize_reader_item_record(value);
    if (item === null) {
      continue;
    }
...
export function build_name_field_glossary_entries(rows: NameFieldRow[]): NameFieldGlossaryEntry[] {
  return rows
    .map((row) => {
      return {
