...
export interface RendererErrorReport extends Record<string, unknown> {
  source: string; // renderer 内部上报来源，不等同 LogManager source
  error: LogError; // 异常本体的唯一结构化载体
  route?: string; // 从当前 renderer 诊断上下文复制
  project?: LogErrorContext; // 只允许轻量摘要
  task?: LogErrorContext; // 只允许轻量摘要
  triggeringEvent?: LogErrorContext; // 记录导致异常的事件头
