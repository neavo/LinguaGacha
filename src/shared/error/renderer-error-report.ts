...
export interface RendererErrorReport extends Record<string, unknown> {
  ...
  diagnostic: ErrorDiagnosticPayload; // diagnostic 是异常本体的唯一结构化载体
  route?: string; // route 从当前 renderer 诊断上下文复制
  project?: ErrorDiagnosticContext; // project 只允许轻量摘要
  task?: ErrorDiagnosticContext; // task 只允许轻量摘要
  triggeringEvent?: ErrorDiagnosticContext; // triggeringEvent 记录导致异常的事件头
  context?: ErrorDiagnosticContext; // context 记录调用点补充的轻量业务上下文
}

export interface RendererErrorReportInput {
  source: string; // source 由调用点选择稳定词表
  error: unknown; // error 是当前捕获到的 JS 异常或 fallback 值
  diagnostic?: ErrorDiagnosticPayload; // diagnostic 用于保留 worker 等边界已生成的异常快照
...
/**
 * renderer 异常报告只通过结构化 diagnostic 进入 Gateway，避免 message / stack / context 多套契约并存。
 */
export function create_renderer_error_report(input: RendererErrorReportInput): RendererErrorReport {
  const diagnostic = input.diagnostic ?? to_error_diagnostic(input.error);
...
export function normalize_renderer_error_report(value: unknown): RendererErrorReport {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    return {
      source: "renderer",
      diagnostic: error_diagnostic_from_message("unknown_renderer_error"),
    };
  }
  const record = value as Record<string, unknown>;
  return {
    source: read_renderer_error_source(record["source"]),
    diagnostic: normalize_error_diagnostic(record["diagnostic"], "unknown_renderer_error"),
