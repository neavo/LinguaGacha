...
const SPECIAL_PUNCTUATION_SET = new Set(["·", "・", "♥"]); // 少量项目约定符号不在 Unicode 标点范围内，但业务上按标点处理
...
export class TextTool {
  ...
  /**
   * 去掉首尾阿拉伯数字，保持历史 TextHelper 的轻量处理口径
   */
...
export const is_cjk_punctuation_character = TextTool.is_cjk_punctuation_character.bind(TextTool); // 兼容旧调用点的函数式导出，真实实现仍收口在 TextTool 类上
...
export const is_punctuation_character = TextTool.is_punctuation_character.bind(TextTool); // 统一标点函数式导出是历史 helper 迁移后的兼容入口
