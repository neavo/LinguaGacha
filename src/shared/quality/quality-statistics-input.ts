...
type QualityStatisticsPrepareTaskInputArgs = {
  ...
  relation_entries?: unknown; // 筛选统计时仍以完整规则集合计算包含关系
  items: Array<Record<string, unknown>>; // cache item 快照，不向 worker 传可变引用
  collect_context_samples?: boolean; // 默认 false，不扩大 GUI/cache 响应
};
...
export function prepare_quality_statistics_task_input(
  ...
  const relation_entries =
    args.relation_entries === undefined
      ? entries
      : ensure_quality_rule_entry_ids(normalize_quality_rule_entries(rule, args.relation_entries));
  /** 统计规则只保留 worker 执行所需的匹配配置。 */
  const to_statistics_rule = (entry: (typeof entries)[number]): QualityStatisticsRuleInput => {
    const pattern_kind =
  ...
  const rules = entries.map(to_statistics_rule);
  ...
  const relation_candidates = relation_entries.flatMap((entry) => {
    const statistics_rule = to_statistics_rule(entry);
    return statistics_rule.pattern_kind === "literal"
      ? [
          {
  ...
  return {
    ...
    collect_context_samples: args.collect_context_samples === true,
  };
}
