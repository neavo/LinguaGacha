...
describe("产品统一入口", () => {
  it("发布态 app.exe 使用 --cli 后的命令参数并以可执行文件目录作为 appRoot", async () => {
    ...
    try {
      ...
      expect_worker_threads_execution(calls.cli[0]?.workerExecution);
  ...
  it("可执行文件名为 cli.exe 但没有 --cli 时仍进入 GUI 入口", async () => {
    ...
    expect_worker_threads_execution(calls.gui[0]?.workerExecution);
 ...
 */
function expect_worker_threads_execution(worker_execution: WorkerPoolExecution | undefined): void {
  expect(worker_execution?.kind).toBe("worker_threads");
  if (worker_execution?.kind !== "worker_threads") {
    return;
  }
  expect(String(worker_execution.workerEntryUrl)).toMatch(/\/worker-entry\.js$/u);
}
