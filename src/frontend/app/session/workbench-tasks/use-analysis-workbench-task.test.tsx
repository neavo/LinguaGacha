...
import { useAnalysisWorkbenchTask } from "@frontend/app/session/workbench-tasks/use-analysis-workbench-task";
...
function Probe(props: {
  ...
  const state = useAnalysisWorkbenchTask();
...
describe("useAnalysisWorkbenchTask", () => {
  let container: HTMLDivElement | null = null;
  let root: Root | null = null;
  let latest_state: ReturnType<typeof useAnalysisWorkbenchTask> | null = null;
  ...
  it("分析完成且存在候选术语时自动弹出导入确认框", async () => {
    api_fetch_mock.mockImplementation(async (path: string) => {
      ...
      if (path === "/api/analysis/candidates/list") {
        return {
          candidate_aggregate: {
            alpha: {
              src: "alpha",
  ...
  it("手动导入遇到重复候选时打开重复确认框且不写入", async () => {
    ...
    api_fetch_mock.mockImplementation(async (path: string) => {
      ...
      if (path === "/api/analysis/glossary-import/preview") {
        return {
  ...
  it("重复候选选择跳过时只提交分析候选消费", async () => {
    ...
    api_fetch_mock.mockImplementation(async (path: string, body?: Record<string, unknown>) => {
      ...
      if (path === "/api/analysis/glossary/import") {
        return {
          accepted: true,
          changes: [
            {
    ...
    expect(api_fetch_mock).toHaveBeenCalledWith("/api/analysis/glossary/import", {
      entries: [
        {
          src: "alpha",
          dst: "旧译名",
