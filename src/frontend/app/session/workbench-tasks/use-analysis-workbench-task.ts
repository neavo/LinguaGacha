...
type AnalysisWorkbenchTaskOptions = Record<string, never>;

export type AnalysisWorkbenchTask = {
...
export function useAnalysisWorkbenchTask(
  ...
  const analysis_waveform_state_ref = useRef(create_empty_task_waveform_state());
  const {
  ...
  const has_unsettled_analysis_waveform_tail = useMemo(() => {
    return has_unsettled_task_waveform_tail(analysis_waveform_history);
  ...
  const clear_analysis_waveform_sampling = useCallback((): void => {
    analysis_waveform_state_ref.current = create_empty_task_waveform_state();
  }, []);
  ...
  const sync_runtime_task_snapshot = useCallback(
    (next_snapshot: AnalysisTaskSnapshot): void => {
      sync_task_snapshot({
        run_revision: next_snapshot.run_revision,
  ...
  const refresh_analysis_task_snapshot = useCallback(async (): Promise<void> => {
    ...
    if (should_defer_task_snapshot_refresh(task_snapshot, "analysis")) {
      return;
    }
  ...
  const commit_prepared_analysis_glossary_import = useCallback(
    async (prepared_import: PreparedAnalysisGlossaryImport): Promise<void> => {
      await commit_project_write({
        operation: WORKBENCH_ANALYSIS_IMPORT_WRITE,
        task_type: "analysis",
        run: async () => {
    ...
    [commit_project_write, push_toast, refresh_task, t],
  );
  ...
  useEffect(() => {
    ...
    const terminal_transition = resolve_task_terminal_transition({
      ...
      is_active_status: is_active_analysis_task_status,
    });
  ...
  return useMemo<AnalysisWorkbenchTask>(() => {
    return {
      analysis_task_display_snapshot,
