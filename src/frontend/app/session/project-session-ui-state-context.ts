...
// 允许页面在项目切换等场景更新快照但不写回旧 session。
type ProjectSessionTableUiStateUpdateOptions = {
  persist?: boolean;
};

// 页面状态 hook 与 session UI 状态之间的唯一表格适配器。
...
// 在读取 session 快照时切断可变引用，避免页面间共享数组对象。
...
// 当前项目 session 内页面 UI 状态的最小读写契约。
export type ProjectSessionUiStateContextValue = {
...
// 不给默认假实现，缺少 Provider 时必须立即暴露接入错误。
export const ProjectSessionUiStateContext = createContext<ProjectSessionUiStateContextValue | null>(
  null,
);

// 页面访问 session UI 状态的唯一入口，避免直接接触内部 Map。
...
export function useProjectSessionTableUiState<FilterState, SortState>(
  ...
  // 只在首帧读取 session 快照，避免后续渲染把本页编辑中的状态覆盖掉。
  ...
  // 同步 ref 与 state，保证恢复滚动只在初始快照阶段生效。
  ...
  // 写入完整快照，避免 session 内留下半更新的筛选、排序或选区组合。
  ...
  // 同步 state/ref/session，保证筛选变更能跨路由恢复。
  ...
  // 先归一化再写入，避免无效列 id 在 session 内长期保留。
  ...
  // 用于项目身份切换，重置本地状态但可按场景跳过旧 session 写回。
