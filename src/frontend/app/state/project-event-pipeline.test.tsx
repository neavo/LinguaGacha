...
import {
  useProjectEventPipeline,
  type ProjectEventPipeline,
  type ProjectChangeEventPayload,
} from "./project-event-pipeline";

let root: Root | null = null;
let container: HTMLDivElement | null = null;

const project_change_event: ProjectChangeEventForState = {
  eventId: "event-1",
  source: "task",
...
function ProjectPipelineProbe(props: {
  ...
  onPipeline: (pipeline: ProjectEventPipeline) => void;
}): JSX.Element | null {
  const pipeline = useProjectEventPipeline({
    projectSnapshot: {
      loaded: true,
      path: "E:/demo/demo.lg",
    },
...
): Promise<ProjectEventPipeline> {
  let pipeline: ProjectEventPipeline | null = null;
  ...
  await act(async () => {
    root?.render(
      <ProjectPipelineProbe
        ...
        refreshProjectStateAfterError={refreshProjectStateAfterError}
...
describe("useProjectEventPipeline", () => {
  afterEach(async () => {
    if (root !== null) {
      await act(async () => {
        root?.unmount();
  ...
  it("无法规范化的项目事件通过统一恢复 runner 回到后端快照", async () => {
    ...
    const pipeline = await render_pipeline(() => null, refresh_project_state_after_error);
