...
describe("useGlossaryPageState", () => {
  ...
  beforeEach(() => {
    ...
    read_quality_rule_snapshot_mock.mockImplementation(async () => ({
  ...
  it("统计刷新中会保留 hit 排序并继续使用旧命中结果", async () => {
    ...
    expect(latest_state?.hit_badge_by_entry_id["苹果::0"]?.hits).toBe(3);
  });
  ...
  it("旧项目保存回包不会在新项目事实刷新后重建当前结果成员", async () => {
    await mount_probe();
    const stale_project_write_result = create_quality_write_result({
      quality: create_glossary_quality(
        [
          ...create_default_glossary_entries(),
          {
    ...
    let resolve_save: (payload: typeof stale_project_write_result) => void = () => {};
    api_fetch_mock.mockReturnValueOnce(
      new Promise<typeof stale_project_write_result>((resolve) => {
        resolve_save = resolve;
      }),
    );
    ...
    await act(async () => {
      run_state.project.path = "E:/demo/other.lg";
      run_state.quality = create_glossary_quality(
        [
          {
            entry_id: "qr:orange",
            src: "橘子",
