...
export function run_quality_statistics_worker_task(
  ...
  const statistics_result = run_quality_statistics_task_sync({
    ...
    collect_context_samples: input.collect_context_samples,
  });
  ...
  const context_samples = statistics_result.context_samples_by_entry_id;
  return {
    phase: "current",
    ...
    request_token: 0,
    updated_at: Date.now(),
    ...(context_samples === undefined
      ? {}
      : {
          context_samples_by_entry_id: Object.fromEntries(
            input.completed_entry_ids.map((entry_id) => [
              entry_id,
              context_samples[entry_id] ?? [],
            ]),
          ),
        }),
  };
