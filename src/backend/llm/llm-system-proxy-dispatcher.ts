...
class SystemProxyDispatcher extends Dispatcher {
  ...
  private async close_proxy_dispatchers(): Promise<void> {
    const dispatchers = [...this.proxy_dispatchers.values()];
    this.proxy_dispatchers.clear();
    await this.settle_proxy_dispatchers(
  ...
  private async destroy_proxy_dispatchers(error: Error | null): Promise<void> {
    ...
    this.proxy_dispatchers.clear();
    await this.settle_proxy_dispatchers(
      dispatchers.map((dispatcher) => dispatcher.destroy(error)),
      "系统代理连接池销毁失败",
    );
  }

  /**
   * 所有代理池都完成释放后再汇总异常，避免单项快速失败留下后台关闭任务。
   */
  private async settle_proxy_dispatchers(
