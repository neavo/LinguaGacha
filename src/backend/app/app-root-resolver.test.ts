...
import { NPM_INITIAL_CWD_ENV_NAME, resolve_app_root } from "./app-root-resolver";
import type { AppRootResolutionEnvironment } from "./app-root-resolver";
...
describe("resolve_app_root", () => {
  ...
  it("找不到应用根标记时返回解析后的 appRoot", () => {
    ...
    const result = resolve_app_root(create_environment({ appRoot: app_root }));
...
function create_environment(
  overrides: Partial<AppRootResolutionEnvironment> = {},
): AppRootResolutionEnvironment {
  return {
    env: {},
    appRoot: process.cwd(),
    platform: "win32",
