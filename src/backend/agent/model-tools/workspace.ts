...
const WORKSPACE_RUN_DESCRIPTION: string = [
  ...
  `- 使用 await ws.emitImage(path) 返回工作区图片，后端统一转换为 WebP。每次最多 ${AGENT_WORKSPACE_RUNTIME_POLICY.imageCount} 张，累计 base64 不超过 ${AGENT_WORKSPACE_RUNTIME_POLICY.imageOutputBytes / 1024 / 1024} MiB。`,
  "- 图片按调用顺序返回，`await` 完成后内容已固定。模型在程序成功返回后看到图片。程序失败时，按执行记录中的路径重新输出图片。",
