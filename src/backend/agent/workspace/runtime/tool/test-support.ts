...
import { create_agent_workspace_data_tool_context, type AgentWorkspaceReadPort } from "./data-tool";
import { execute_agent_workspace_data_tool, type AgentWorkspaceDataToolName } from "./registry";

/** 测试验证数据工具遵循 contract 上限，而不复制当前可调数值。 */
export const WORKSPACE_QUERY_PAGE_MAX = Number(
  read_json_record(AGENT_WORKSPACE_CONTRACT["limits"])["query_page_max"],
);

/** 用真实类型化函数和最小只读文件端口验证数据工具的公开结果。 */
export async function execute_workspace_tool(
  ...
  return await execute_agent_workspace_data_tool(
    name,
    create_agent_workspace_data_tool_context(workspace),
    args,
  );
}
