...
describe("ws.tool.groupQualityRuleEntries 数据工具", () => {
  ...
  it("按领域声明的 16 条上限拆分超大强 component 并保留身份与跨组边", async () => {
    ...
    const result = read_json_record(
      await execute_workspace_tool("groupQualityRuleEntries", { kind: "glossary", entries }, {}),
    );
    const groups = result["groups"] as JsonRecord[];
  ...
  it("分页返回完整结构组计数和下一位置", async () => {
    const result = read_json_record(
      await execute_workspace_tool(
        "groupQualityRuleEntries",
        {
          kind: "text_preserve",
          entries: [
            relation_candidate("first", "a"),
            relation_candidate("second", "b"),
            relation_candidate("third", "c"),
          ],
          offset: 1,
          limit: 1,
        },
    ...
    expect(result).toMatchObject({ total_group_count: 3, next_offset: 2 });
    expect(result["groups"]).toEqual([
  ...
  it("拒绝超过 contract 上限的分页大小", async () => {
    await expect(
      execute_workspace_tool(
        "groupQualityRuleEntries",
        { kind: "text_preserve", entries: [], limit: WORKSPACE_QUERY_PAGE_MAX + 1 },
        {},
      ),
    ).rejects.toThrow();
  });
