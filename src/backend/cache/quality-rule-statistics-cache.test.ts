...
function create_worker(): AnalysisWorker & {
  ...
  const run = vi.fn(async (task: AnalysisWorkerTask) => {
    if (task.type !== "quality_rule_statistics") throw new Error("unexpected task");
    return {
...
describe("QualityRuleStatisticsCache", () => {
  it("重复和并发读取复用同一个 worker 结果", async () => {
    const port = create_cache_read_port();
    const worker = create_worker();
    const cache = new QualityRuleStatisticsCache({ cache: port, workerClient: worker });
  ...
  it("缓存命中不读取质量规则或 item，不计算内容签名", async () => {
    const port = create_cache_read_port();
    const cache = new QualityRuleStatisticsCache({ cache: port, workerClient: create_worker() });
    await cache.read("glossary");
