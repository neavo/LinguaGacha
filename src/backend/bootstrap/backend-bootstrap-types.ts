...
export interface BackendBootstrapStartResult {
  ...
  backendServices: BackendServices; // backendServices 只暴露给同进程入口适配器，不进入 renderer 协议
...
export interface BackendBootstrapOptions {
  ...
  workerExecution: BackendWorkerExecution; // workerExecution 固定 Backend worker 执行配置，避免服务层自行回退或探测入口。
}
