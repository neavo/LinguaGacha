...
export interface TranslationLimiterLease {
...
interface PendingRequest {
  resolve: (lease: TranslationLimiterLease) => void; // 资格发放后结算 acquire Promise
...
/**
 * 翻译限流器，统一发放 LLM work unit 请求资格并保护外部模型服务节奏。
 */
export class TranslationLimiter {
  ...
  /**
   * 初始化限流参数；TranslationLimiter 只接收最终并发值，不再解释 concurrency_limit == 0。
   */
   ...
   */
  public async acquire(signal: AbortSignal): Promise<TranslationLimiterLease> {
    if (signal.aborted) {
      throw this.create_abort_error();
    }
    return await new Promise<TranslationLimiterLease>((resolve, reject) => {
...
/**
 * LimiterPool 按模型资源键复用 TranslationLimiter，让后台任务共享同一外部请求节奏
 */
export class LimiterPool {
  private shared_limiter: { key: string; limiter: TranslationLimiter } | null = null; // 当前模型资源池的唯一缓存

  /**
   * 解析当前模型对应 limiter；影响外部资源池的字段变化时自然切换新 limiter
   */
  public resolve(model: LimiterModelRecord): TranslationLimiter {
    ...
    const limiter = new TranslationLimiter({
