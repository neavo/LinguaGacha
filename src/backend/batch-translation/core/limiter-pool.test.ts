...
import { TranslationLimiter, resolve_effective_concurrency_limit } from "./limiter-pool";

describe("TranslationLimiter", () => {
  ...
  it("无 RPM 时先填满并发，后续请求按隐藏 RPS 补充启动资格", async () => {
    ...
    try {
      vi.setSystemTime(0);
      const limiter = new TranslationLimiter({ max_concurrency: 2, rpm_limit: 0 });
  ...
  it("并发槽释放后才允许后续请求进入", async () => {
    ...
    try {
      vi.setSystemTime(0);
      const limiter = new TranslationLimiter({ max_concurrency: 1 });
      const controller = new AbortController();
      const first = await limiter.acquire(controller.signal);
  ...
  it("有 RPM 时只按 RPM 平滑发放请求资格", async () => {
    ...
    try {
      vi.setSystemTime(0);
      const limiter = new TranslationLimiter({ max_concurrency: 2, rpm_limit: 60 });
      const controller = new AbortController();
      const first = await limiter.acquire(controller.signal);
      let second_acquired = false;
      const second = limiter.acquire(controller.signal).then((lease) => {
