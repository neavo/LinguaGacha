...
describe("PromptBuilder", () => {
  it("每个 item 只生成一条 JSONL，text 内换行由 JSON 转义承载", async () => {
    ...
    const result = builder.generate_prompt(
      [{ request_id: 3, item_index: 0, text_src: "甲\n乙", actor_src: null }],
      "text",
      [],
      [],
    );
    const input = result.messages[1]?.content ?? "";
    expect(input).toContain('{"id":3,"text":"甲\\n乙"}');
    expect(input.match(/\{"id":/gu)).toHaveLength(1);
  });
  ...
  it.each([
    ["ZH", "翻译前缀", "中文", "规则分析", "输出 JSONLINE"],
    ["EN", "Translation prefix", "Chinese", "Rule analysis", "Return JSONLINE"],
  ] as const)(
    "%s 自定义规则与增强开关独立组合，输出约束保持最后",
    async (app_language, prefix, target_language, enhancement, suffix) => {
      const builtin_root = await create_template_root();
      for (const prompt_enhancement_enable of [true, false]) {
        const builder = new PromptBuilder(
          builtin_root,
          { app_language, source_language: "JA", target_language: "ZH", prompt_enhancement_enable },
          create_quality_snapshot({
        ...
        expect(result).toContain(`${prefix}\n自定义规则：${target_language}`);
        expect(result.includes(enhancement)).toBe(prompt_enhancement_enable);
        if (prompt_enhancement_enable) {
          expect(result).toContain(`${enhancement}\n\n${suffix}`);
        }
        expect(result).toMatch(/\{"id":<ID>,"text":"[^\n]+"\}\n```$/u);
      }
    },
  );
  ...
  it("公开提示词只格式化 runner 已激活的术语", async () => {
    ...
    const result = builder.generate_prompt([create_line({ text_src: "abc foo" })], "text", [], []);
  ...
  it("含姓名请求使用 actor/text 格式并写入已激活姓名术语", async () => {
    ...
    const result = builder.generate_prompt(
      [
        create_line({ request_id: 0, text_src: "こんにちは", actor_src: "虎鉄" }),
        create_line({ request_id: 1, text_src: "地の文", actor_src: null }),
      ],
      "actor_text",
      [],
      [],
    ...
    expect(result.messages[0]?.content).toMatch(/"actor":"[^"\n]+","text":"[^"\n]+"/u);
    expect(result.messages[1]?.content).toContain('{"id":0,"actor":"虎鉄","text":"こんにちは"}');
    expect(result.messages[1]?.content).toContain('{"id":1,"actor":null,"text":"地の文"}');
...
async function write_template(
  ...
  const dir = path.join(builtin_root, "translation_prompt", "template", language);
...
function create_line(overrides: {
  ...
  return {
    request_id: overrides.request_id ?? 0,
    item_index: 0,
