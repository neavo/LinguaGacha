...
class WorkUnitWorkerEntry {
  ...
  private readonly runner: TranslationWorkUnitRunner; // 持有提示词、pipeline 与响应处理
  ...
  /**
   * workerData 由 TranslationWorkerPool 注入，只包含 work unit 需要的资源根
   */
  public constructor(options: WorkUnitWorkerData, port: MessagePort) {
    this.port = port;
    this.llm_client = new WorkerLLMClient(port);
    this.runner = new TranslationWorkUnitRunner(options.builtinRoot, this.llm_client);
  }
  ...
  /**
   * 每条消息独立 AbortController，迟到结果由 BatchTranslationRunner 的 run_id 再隔离
   */
  private async run_message(
    ...
    try {
      const data = await this.runner.execute_unit(message.unit, controller.signal);
      this.port.postMessage({
        type: "result",
        id: message.id,
        result: { ok: true, data },
