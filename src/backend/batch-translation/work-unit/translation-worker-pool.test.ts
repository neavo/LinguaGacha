...
import { TranslationWorkerPool } from "./translation-worker-pool";

const cleanup_roots: string[] = []; // 记录测试创建的临时内置资产根和 worker 文件目录

describe("TranslationWorkerPool", () => {
  afterEach(async () => {
    vi.restoreAllMocks();
...
parentPort?.on("message", (message) => {
  ...
  if (message.type === "llm_result") {
    ...
    parentPort?.postMessage({
      type: "result",
      id: execution.id,
      result: { ok: true, data: { unit_id: execution.unit.unit_id, kind: "translation", outcome: "success", metrics: { input_tokens: message.result.data.input_tokens, reasoning_tokens: 0, output_tokens: message.result.data.output_tokens }, output: { kind: "translation", items: [] }, logs: [], from_worker: true, llm: message.result.data } },
    });
  }
});
`,
    ...
    try {
      await expect(
        ...
        kind: "translation",
        outcome: "success",
        metrics: { input_tokens: 3, reasoning_tokens: 0, output_tokens: 4 },
        output: { kind: "translation", items: [] },
        logs: [],
        from_worker: true,
        llm: {
          response_think: "",
