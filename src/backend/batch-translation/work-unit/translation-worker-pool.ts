...
export class TranslationWorkerPool implements WorkUnitExecutor {
  ...
  /**
   * 后台任务 unit 走统一 enqueue，TranslationWorkerPool 不读取任务领域状态。
   */
  ...
  /**
   * BackendServices 关闭 时拒绝在途任务并终止 worker，防止线程和 Promise 泄漏。
   */
  public async dispose(): Promise<void> {
    ...
    if (terminate_errors.length > 0) {
      throw new AggregateError(
        terminate_errors,
        "Failed to terminate TranslationWorkerPool workers.",
      );
    }
  }
  ...
  private create_slot(): WorkerSlot {
    if (this.execution.kind !== "worker_threads") {
      throw new Error(
        "TranslationWorkerPool requires worker_threads mode to create a worker slot.",
      );
    }
  ...
  /**
   * 成功值和传输错误在 TranslationWorkerPool 边界统一完成，BatchTranslationRunner 只识别 executor 结果。
   */
  private settle_task(
    task: PendingTask,
    message: {
  ...
  /**
   * TranslationWorkerPool 生命周期错误集中生成，调用方只按稳定 code 判断资源是否已释放。
   */
  private create_disposed_error(): AppError {
    return new AppError("runtime.disposed", {
      public_details: { resource: "TranslationWorkerPool" },
      diagnostic_context: {
        in_flight_count:
