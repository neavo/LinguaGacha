 ...
 */
export interface TranslationTokenMetric {
  token_count: number;
  line_count: number;
}
 ...
 */
export interface TranslationTokenCountInput {
  cache_key: string;
  text: string;
}
 ...
 */
export interface TranslationTokenCountResult {
  cache_key: string;
  token_count: number;
}

/**
 * 统计非空行数，保持与旧 BatchTranslationRunner 切块 line_limit 同一口径。
 */
 ...
 */
export class TranslationTokenMetricCache {
  private readonly capacity: number; // LRU 上限，避免大型项目反复规划后无限增长。
  private readonly cache = new Map<string, TranslationTokenMetric>(); // 使用 Map 插入顺序表达 LRU。
   ...
   */
  public get(cache_key: string): TranslationTokenMetric | null {
   ...
   */
  public set(cache_key: string, metric: TranslationTokenMetric): void {
    this.cache.delete(cache_key);
