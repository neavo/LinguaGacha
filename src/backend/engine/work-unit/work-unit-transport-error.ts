import { AppError, type LogError } from "../../../shared/error";

/**
 * worker 或 LLM adapter 传输失败时使用专门错误，翻译 chunk 可走可恢复重试
 */
export class WorkUnitExecutorTransportError extends AppError {
  ...
  public constructor(error: LogError, cause_error: unknown) {
    super("worker.failed", { cause: cause_error, diagnostic_context: { failure: error } });
