...
describe("TaskSnapshotBuilder", () => {
  ...
  it("用数据库进度事实和运行态组装任务快照", async () => {
    ...
    task_run_state.begin_task("translation", { kind: "items", item_ids: [101] });
    task_run_state.set_request_in_flight_count("translation", 2);
    const builder = new TaskSnapshotBuilder(database, task_run_state, session_state);
