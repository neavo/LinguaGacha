...
import {
  BACKEND_API_HOST,
  BACKEND_API_PORT_MAX,
  BACKEND_API_PORT_MIN,
  allocate_backend_api_port,
...
describe("api-port-allocator", () => {
  it("生成固定回环地址的 Backend API base URL", () => {
    expect(build_backend_api_base_url(38191)).toBe("http://127.0.0.1:38191");
  });
  ...
  it("分配高位范围内可重新绑定的本地端口", async () => {
    ...
    expect(port).toBeGreaterThanOrEqual(BACKEND_API_PORT_MIN);
    expect(port).toBeLessThanOrEqual(BACKEND_API_PORT_MAX);
  });
