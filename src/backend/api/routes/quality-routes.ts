...
export function register_quality_routes(context: ApiRouteContext): void {
  ...
  context.postJson("/api/quality/statistics/view", (body) => statistics.read(body));
  context.postJson("/api/quality/rules/view", (body) => query.read_quality_rule_view(body));
  context.postJson("/api/quality/prompts/view", (body) => query.read_prompt_view(body));
