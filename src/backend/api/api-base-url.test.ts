...
describe("Backend API 地址契约", () => {
  it("构造本机 Backend API base URL", () => {
    expect(build_backend_api_base_url(38191)).toBe("http://127.0.0.1:38191");
  });

  it("标准化 base URL 末尾斜杠", () => {
    expect(normalize_backend_api_base_url(" http://127.0.0.1:38191/// ")).toBe(
      "http://127.0.0.1:38191",
    );
  });

  it("通过启动参数传递 preload 可读取的 Backend API 地址", () => {
    const argument = build_backend_api_base_url_argument("http://127.0.0.1:38191/");
