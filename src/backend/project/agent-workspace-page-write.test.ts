...
const intent = (page = 1): AgentWorkspacePageUpdateIntent => ({
  file_path: "book.pdf",
  page,
  fp: agent_workspace_page_fingerprint("book.pdf", document.digest, document.pages[page - 1]!),
...
it("同一文档按页隔离旧指纹和重复写入，拒绝回执定位原页", () => {
  const result = resolve_agent_workspace_page_updates(
...
it("页面正文、核对和说明一并替换，恢复原文与无变化提交使用同一入口", () => {
  const saved = resolve_agent_workspace_page_updates([intent()], records).changes[0]!.page;
  ...
  const fp = agent_workspace_page_fingerprint("book.pdf", current.digest, saved);
  expect(
    resolve_agent_workspace_page_updates(
      [{ ...intent(), fp }],
  ...
  const restored = resolve_agent_workspace_page_updates(
...
it("页面保存允许空译稿与全部省略，非法正文和来源变化只拒绝相应页", () => {
  ...
  expect(
    resolve_agent_workspace_page_updates([{ ...intent(), page: 4 }], records).rejected,
  ).toMatchObject([{ page: 4, reason: "target_missing" }]);
});

it("页指纹绑定来源和页面事实，邻页修改不影响当前页", () => {
  const copy = structuredClone(document);
  const first = agent_workspace_page_fingerprint("book.pdf", document.digest, document.pages[0]!);
  copy.pages[1]!.notes = "继续第 2 页";
  expect(agent_workspace_page_fingerprint("book.pdf", copy.digest, copy.pages[0]!)).toBe(first);
  expect(agent_workspace_page_fingerprint("other.pdf", copy.digest, copy.pages[0]!)).not.toBe(
    first,
  );
  expect(agent_workspace_page_fingerprint("book.pdf", "b".repeat(64), copy.pages[0]!)).not.toBe(
    first,
  );
  copy.pages[0]!.reviewed = true;
  expect(agent_workspace_page_fingerprint("book.pdf", copy.digest, copy.pages[0]!)).not.toBe(first);
  copy.pages[0]!.reviewed = false;
  copy.pages[0]!.translation = { kind: "keep", reason: "无需翻译" };
  const kept = agent_workspace_page_fingerprint("book.pdf", copy.digest, copy.pages[0]!);
  expect(kept).not.toBe(first);
  copy.pages[0]!.translation.reason = "原页只有插画";
  expect(agent_workspace_page_fingerprint("book.pdf", copy.digest, copy.pages[0]!)).not.toBe(kept);
