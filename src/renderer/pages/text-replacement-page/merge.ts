...
function build_key(src_fold: string, src_norm: string): TextReplacementMergeKey {
  return `${src_fold}::${src_norm}`;
}

function merge_into_base(base: TextReplacementEntry, other: TextReplacementEntry): boolean {
  let changed = false;
