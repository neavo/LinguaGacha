...
type DesktopRuntimeEventStreamOptions = Parameters<typeof useDesktopRuntimeEventStream>[0];
...
function EventStreamProbe(props: {
  options: Omit<DesktopRuntimeEventStreamOptions, "schedulerRef">;
}): JSX.Element | null {
  const scheduler_ref = useRef<DesktopRuntimeRefreshScheduler | null>(null);
...
describe("useDesktopRuntimeEventStream", () => {
  afterEach(async () => {
    if (root !== null) {
      await act(async () => {
        root?.unmount();
  ...
  it("项目事件管线失败时用摘要化 payload 上报 renderer 错误", async () => {
    ...
    render_event_stream({
      ...
      recovery: {
        ...
        refresh_task_after_runtime_error: vi.fn(async () => undefined),
    ...
    await wait_for_condition(() => report_runtime_error.mock.calls.length > 0);
    const report_args = report_runtime_error.mock.calls[0]?.[1];
