...
type CommitterProbeProps = {
  ...
  reportRuntimeError: (error: unknown, args: unknown) => void;
  refreshProjectRuntimeAfterError: (
    reason: string,
    triggering_event: unknown,
    recovery_context?: unknown,
  ) => Promise<void>;
  onCommitter: (committer: ProjectMutationCommitter) => void;
};

// CommitterProbe 收口测试中的共享步骤，保证断言只关注当前行为。
function CommitterProbe(props: CommitterProbeProps): JSX.Element | null {
  const committer = useProjectMutationCommitter({
    applyProjectMutationChanges: props.applyProjectMutationChanges,
    recovery: {
      report_runtime_error: props.reportRuntimeError,
...
async function wait_for_expectation(predicate: () => boolean): Promise<void> {
  // mutation catch 链路跨多个 Promise 边界，测试只等待公开恢复调用出现。
  ...
  throw new Error("等待 mutation 提交状态收敛失败。");
}

describe("useProjectMutationCommitter", () => {
  afterEach(async () => {
    if (root !== null) {
      await act(async () => {
        root?.unmount();
  ...
  it("通过显式 operation 提交 mutation 并返回规范化结果", async () => {
    const runtime: { committer: ProjectMutationCommitter | null } = { committer: null };
    const apply_project_mutation_changes = vi.fn(async () => undefined);
    ...
    await act(async () => {
      root?.render(
        <CommitterProbe
          ...
          refreshProjectRuntimeAfterError={vi.fn(async () => undefined)}
          onCommitter={(next_committer) => (runtime.committer = next_committer)}
        />,
      );
    });
    ...
    if (committer === null) {
      throw new Error("mutation committer 未初始化。");
    }
    ...
    expect(apply_project_mutation_changes).toHaveBeenCalledWith({
      accepted: true,
      changes: [],
    });
  ...
  it("先执行提交前准备再回灌项目变更", async () => {
    ...
    const apply_project_mutation_changes = vi.fn(async () => {
  ...
  it("提交前准备失败时不回灌项目变更并进入恢复", async () => {
    ...
    const apply_project_mutation_changes = vi.fn(async () => undefined);
    const report_runtime_error = vi.fn();
    ...
    await act(async () => {
      root?.render(
        <CommitterProbe
          applyProjectMutationChanges={apply_project_mutation_changes}
          reportRuntimeError={report_runtime_error}
          refreshProjectRuntimeAfterError={refresh_project_runtime_after_error}
          onCommitter={(next_committer) => (runtime.committer = next_committer)}
        />,
      );
    });
    ...
    expect(apply_project_mutation_changes).not.toHaveBeenCalled();
    expect(report_runtime_error).toHaveBeenCalledWith(
      original_error,
  ...
  it("回灌失败时等待项目恢复完成并重新抛出原始错误", async () => {
    ...
    const report_runtime_error = vi.fn();
    const refresh_project_runtime_after_error = vi.fn(() => recovery_promise);
    container = document.createElement("div");
    root = createRoot(container);
    ...
    await act(async () => {
      root?.render(
        <CommitterProbe
          applyProjectMutationChanges={async () => {
            throw original_error;
          }}
          reportRuntimeError={report_runtime_error}
          refreshProjectRuntimeAfterError={refresh_project_runtime_after_error}
    ...
    await wait_for_expectation(() => refresh_project_runtime_after_error.mock.calls.length === 1);
