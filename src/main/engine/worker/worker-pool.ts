...
export class WorkerPool implements WorkerExecutor {
  ...
  /**
   * 后台任务 unit 走 worker 执行，返回结构由 runner 保证
   */
  public async execute_unit(
    unit: WorkUnit,
    signal: AbortSignal,
  ): Promise<WorkerExecutionResult> {
    return (await this.enqueue_unit(unit, signal)) as WorkerExecutionResult;
  }
  ...
  > {
    return (await this.enqueue_translate_single(body, signal)) as Record<
      string,
      ApiJsonValue
    > & {
      logs?: Array<{ level: "info" | "warning" | "error"; message: string }>;
   ...
   */
  private enqueue_unit(unit: WorkUnit, signal: AbortSignal): Promise<unknown> {
    return this.enqueue(unit, null, signal);
  }

  /**
   * 单条翻译仍是工具调用，不进入后台任务 unit 协议
   */
  private enqueue_translate_single(
    body: Record<string, ApiJsonValue>,
    signal: AbortSignal,
  ): Promise<unknown> {
    return this.enqueue(null, body, signal);
  }
  ...
  private enqueue(
    ...
    return new Promise((resolve, reject) => {
      if (this.direct_runner !== null) {
        const task_promise =
          unit === null
            ? this.direct_runner.translate_single(translate_single_body ?? {}, signal)
            : this.direct_runner.run(unit, signal);
      ...
      slot.worker.postMessage(
        task.unit === null
          ? {
              ...
              body: task.translate_single_body,
            }
