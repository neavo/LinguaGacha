...
import { WorkUnitRunner, type WorkUnitRunnerOptions } from "./worker-runner";
...
interface WorkerTranslateSingleMessage {
  id: string;
...
class TaskWorkerEntry {
  ...
  /**
   * workerData 由 WorkerPool 注入，只包含 work unit 需要的资源根
   */
   ...
   */
  private async run_message(message: WorkerExecuteMessage | WorkerTranslateSingleMessage): Promise<void> {
...
const entry = new TaskWorkerEntry(workerData as WorkUnitRunnerOptions); // 顶层入口必须立即绑定 parentPort，worker_threads 加载后即可接收池派发消息
