...
export class TaskEngine {
  ...
  private async run_translation(
    ...
    try {
      ...
      if (!handle.signal.aborted) {
        this.log_task_error("翻译任务执行失败。", error);
      }
    } finally {
  ...
  private async run_analysis(
    ...
    try {
      ...
      final_status = handle.signal.aborted ? "IDLE" : "ERROR";
      if (!handle.signal.aborted) {
        this.log_task_error("分析任务执行失败。", error);
      }
    } finally {
  ...
  private async run_retranslate(
    ...
    try {
      ...
      final_status = handle.signal.aborted ? "IDLE" : "ERROR";
      if (!handle.signal.aborted) {
        this.log_task_error("重翻任务执行失败。", error);
      }
    } finally {
  ...
  private async commit_translation_entries(
    ...
    this.task_store.commit_translation_batch({
  ...
  private async commit_analysis_entries(
    ...
    next_progress = TaskProgressSnapshotTool.with_elapsed(next_progress);
    this.task_store.commit_analysis_batch({
    ...
    const message =
      status === "DONE"
        ...
        : status === "IDLE"
          ? `${task_label}已停止。`
          : `${task_label}失败。`;
    this.append_log(status === "ERROR" ? "warning" : "info", message, "task-engine");
  }
  ...
  private emit_work_unit_logs(
    ...
    for (const entry of logs) {
      this.append_log(entry.level, entry.message, "task-worker");
    }
  }
