...
export class TaskRuntimePublisher {
  ...
  /**
   * 命令受理后立即进入 REQUEST，并发布完整快照
   */
  public async begin_task(task_type: TaskType, item_ids: number[] = []): Promise<void> {
    this.runtime_state.begin_task(task_type, item_ids);
   ...
   */
  public async publish_status(task_type: TaskType, status: string, busy: boolean): Promise<void> {
    if (!busy) {
  ...
  private resolve_snapshot_task_type(snapshot: TaskRuntimeStatePayload): TaskType {
    ...
    return task_type === "analysis" || task_type === "retranslate" ? task_type : "translation";
  }
}
