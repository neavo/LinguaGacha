...
- 信頼できるAIモデルインターフェースを取得。おすすめ：
  - [ [DeepSeek API](https://github.com/neavo/LinguaGacha/wiki/DeepSeek) ] (GPU不要)
- ソーステキストを準備：
  - `字幕`/`電子書籍`は通常、前処理が不要
...
- アプリケーションを起動：
  ...
  - `AGENT` でモデルを選択し、画面上のプリセット指示を順にクリックして以下の手順を実行：
    - `用語抽出`　`👈👈 任意ですが推奨。翻訳品質に大きく影響します`
    - `全文翻訳`
    - `自動校正`　`👈👈 任意ですが推奨。翻訳品質に大きく影響します`
  - `AGENT` で訳文生成をクリック
...
## 最近の更新 📅
- 20260920 v0.122.2
  - 修正と改善 [#908](../../issues/908) [#912](../../issues/912) [#913](../../issues/913) [#914](../../issues/914)
...
- 20260918 v0.122.0
  - `.pdf` ファイルに対応 [#895](../../issues/895)
    - `AGENT` モードでのみ翻訳できます
    - `通常テキスト` と `画像テキスト` に対応
  - `OpenCode Go` の特殊フィールド規則に対応 [#894](../../issues/894)
  - 修正と改善 [#877](../../issues/877) [#888](../../issues/888) [#890](../../issues/890) [#891](../../issues/891) [#892](../../issues/892) [#893](../../issues/893) [#896](../../issues/896) [#897](../../issues/897) [#898](../../issues/898) [#899](../../issues/899)
