    ...
    def fix(cls, dst: str) -> str:
        ...
        for i, char in enumerate(dst):
            if char in HangeulFixer.RULE_ONOMATOPOEIA:
                ...
                is_prev_hangeul = prev_char is not None and TextHelper.KO.hangeul(
                    prev_char
                )
                is_next_hangeul = next_char is not None and TextHelper.KO.hangeul(
                    next_char
                )

                # 如果前后字符中有谚文，则保留当前字符
                if is_prev_hangeul or is_next_hangeul:
                    result.append(char)
            else:
                # 非拟声词字符直接保留
                result.append(char)
