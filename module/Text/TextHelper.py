...
class TextHelper:
    ...
    def is_punctuation(cls, char: str) -> bool:
        return (
            cls.is_cjk_punctuation(char)
            or cls.is_latin_punctuation(char)
            or cls.is_special_punctuation(char)
        )
    ...
    def split_by_punctuation(cls, text: str, split_by_space: bool) -> list[str]:
        ...
        for char in text:
            if cls.is_punctuation(char) or (
                split_by_space and char in (chr(0x0020), chr(0x3000))
            ):
                if current_segment != []:
    ...
    def get_encoding(
        ...
        add_sig_to_utf8: bool = True,
    ) -> str:
        encoding: str = "utf-8"

        try:
            if path is not None:
                res = charset_normalizer.from_path(path).best()
                if res is not None:
                    encoding = res.encoding
            elif content is not None:
                res = charset_normalizer.from_bytes(content).best()
                if res is not None:
                    encoding = res.encoding
        except Exception:
            pass
        ...
        # 如果需要添加 BOM 标识
        if add_sig_to_utf8 and (encoding in ("utf_8", "utf-8")):
            encoding = "utf-8-sig"
