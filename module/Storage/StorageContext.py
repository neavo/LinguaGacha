...
class StorageContext(Base):
    ...
    instance: "StorageContext | None" = None
    lock = threading.Lock()
    ...
    def get_project_status(self) -> "Base.ProjectStatus":
        ...
        status_str = self.db.get_meta("project_status", Base.ProjectStatus.NONE)
        return Base.ProjectStatus(status_str)

    def set_project_status(self, status: "Base.ProjectStatus") -> None:
        """设置当前工程的项目状态"""
        if self.db is not None:
            self.db.set_meta("project_status", status.value)

    def get_translation_extras(self) -> dict:
        """获取翻译进度额外数据"""
        if self.db is None:
            return {}
        extras = self.db.get_meta("translation_extras", {})
        return extras if isinstance(extras, dict) else {}

    def set_translation_extras(self, extras: dict) -> None:
        """设置翻译进度额外数据"""
        if self.db is not None:
            self.db.set_meta("translation_extras", extras)
