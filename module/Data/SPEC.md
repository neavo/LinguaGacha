...
- `Proofreading/` 由 `api/v2/Application/ProofreadingAppService.py` 组合使用；它依赖 `DataManager` 读取当前工程、提交 mutation，并自行维护条目范围判断、revision 与重检逻辑。
...
| 工作台文件增删改、批量文件操作与运行态事实刷新 | `DataManager` -> `ProjectFileService` / `ProjectRuntimeService` |
...
- 文件重排、增删改都先完成持久化与预过滤；Electron 主路径通过 `project.patch` 失效通知与本地 selector 重建工作台视图。
...
- `Proofreading/` 与 `Extra/` 采用独立服务分层，不要为了省事回退成页面或 API 直接拼数据。
