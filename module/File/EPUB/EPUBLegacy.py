        ...
        with zip_reader.open(path) as reader:
            ...
            for dom in bs.find_all(self.EPUB_TAGS):
                ...
                if item_obj.get_src() in str(dom):
                    ...
                    # 如果 effective_dst 包含 HTML 结构（如完整的带标签元素），则用解析后的 DOM 替换
                    # 否则直接设置文本内容
                    new_dom = BeautifulSoup(effective_dst, "html.parser")
                    if new_dom and new_dom.contents and hasattr(new_dom.contents[0], "name") and new_dom.contents[0].name:
